...
class GGUFReader:
    ...
    def _build_tensors(self, start_offs: int, fields: list[ReaderField]) -> None:
        tensors = []
        # Custom IDs are local to each file; names identify the tensor layout.
        registry = {
            "Q1_sym_g32_sq4_sg8": GGMLQuantizationType.Q1_SYM32K,
            "Q2_sym_g32_sq4_sg8": GGMLQuantizationType.Q2_SYM32K4,
            "Q4_sym_g16_sq8_sg8": GGMLQuantizationType.Q4_SYM16K,
            "Qt_sym_ms_g32_sq4_sg8": GGMLQuantizationType.QT_MS32K4,
            "Q4_sym_g16_f16": GGMLQuantizationType.Q4_SYM16F,
        }
        custom_types = {}
        declarations = self.fields.get("general.tensor_types")
        if declarations is not None:
            if declarations.types != [GGUFValueType.ARRAY, GGUFValueType.STRING]:
                raise ValueError("general.tensor_types must be an array of strings")
            for declaration in declarations.contents():
                identifier, separator, name = declaration.partition("=")
                if not separator or not identifier.isdecimal():
                    raise ValueError(f"Invalid custom tensor type: {declaration}")
                identifier = int(identifier)
                if identifier in custom_types:
                    raise ValueError(f"Duplicate custom tensor type ID: {identifier}")
                custom_types[identifier] = registry.get(name)
        ...
        for field in fields:
            ...
            identifier = int(raw_dtype[0])
            if identifier in custom_types:
                ggml_type = custom_types[identifier]
                if ggml_type is None:
                    raise ValueError(f"Unsupported custom tensor type ID: {identifier}")
            else:
                ggml_type = GGMLQuantizationType(identifier)
