        ...
        for (    int i_tg = 0; i_tg < (int) n_tg.size(); ++i_tg) {
            for (int i_pl = 0; i_pl < (int) n_pl.size(); ++i_pl) {
                ...
                if (is_pp_shared) {
                    ...
                    // Recurrent/hybrid models cannot roll back state, so the
                    // dummy-token + seq_rm trick would leave seq 0's position
                    // permanently advanced (M-RoPE then rejects the TG batch).
                    // seq_cp materializes recurrent state directly, so skip it.
                    if (!params.kv_unified && !llama_model_is_recurrent(model) && !llama_model_is_hybrid(model)) {
