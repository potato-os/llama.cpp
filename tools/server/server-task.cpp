...
task_params server_task::params_from_json_cmpl(
    ...
    params.pin_checkpoint_from_end = json_value(data, "pin_checkpoint_from_end", 0);
    ...
    params.include_raw_generation = json_value(data, "include_raw_generation", false);
    if (data.contains("repeat_stop") && data.at("repeat_stop").is_object()) {
        const auto & rs = data.at("repeat_stop");
        params.repeat_stop.enabled     = true;
        params.repeat_stop.windows     = json_value(rs, "windows",     params.repeat_stop.windows);
        params.repeat_stop.min_tokens  = json_value(rs, "min_tokens",  params.repeat_stop.min_tokens);
        params.repeat_stop.min_repeats = json_value(rs, "min_repeats", params.repeat_stop.min_repeats);
        params.repeat_stop.coverage    = json_value(rs, "coverage",    params.repeat_stop.coverage);
        params.repeat_stop.stride      = json_value(rs, "stride",      params.repeat_stop.stride);
    }
    if (data.contains("ngram_penalty") && data.at("ngram_penalty").is_object()) {
        const auto & np = data.at("ngram_penalty");
        params.ngram_penalty_spans   = json_value(np, "spans",   std::vector<std::string>{});
        params.ngram_penalty_start_n = json_value(np, "start_n", params.ngram_penalty_start_n);
        params.ngram_penalty_scale   = json_value(np, "scale",   params.ngram_penalty_scale);
        params.ngram_penalty_max     = json_value(np, "max",     params.ngram_penalty_max);
    }
...
json server_task_result_cmpl_final::to_json_oaicompat_chat() {
    ...
    if (include_raw_generation) {
        res["raw_generation"] = raw_generation;
    }
...
json server_task_result_cmpl_final::to_json_oaicompat_chat_stream() {
    ...
    if (include_raw_generation) {
        // The unparsed generation, for clients that log model output verbatim.
        final_chunk["raw_generation"] = raw_generation;
    }
    if (!repetition.is_null()) {
        final_chunk["repetition"] = repetition;
    }
