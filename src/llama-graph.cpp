...
llm_graph_context::llm_graph_context(const llm_graph_params & params) :
    ...
    n_expert_used    (cparams.warmup && !params.hot_cache ? hparams.n_expert : hparams.n_expert_used),
...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    ggml_tensor * hot_physical = nullptr;
    if (hot_cache) {
        GGML_ASSERT(arch == LLM_ARCH_QWEN35MOE && n_expert == 256 && n_expert_used == 8);
        GGML_ASSERT(!exp_probs_b && hparams.n_expert_groups <= 1);
        auto * route = llama_hot_route(hot_cache, ctx0, logits, il);
        selected_experts = ggml_view_2d(ctx0, route, 8, n_tokens, route->nb[1], 0);
        hot_physical = ggml_view_2d(ctx0, route, 8, n_tokens, route->nb[1], 8*sizeof(int32_t));
    } else {
    ...
    if (hot_cache && n_tokens > 1) {
        GGML_ASSERT(gate_exps && up_exps && down_exps && !gate_up_exps);
        gate_exps = llama_hot_union_weight(hot_cache, gate_exps, il);
        up_exps = llama_hot_union_weight(hot_cache, up_exps, il);
        down_exps = llama_hot_union_weight(hot_cache, down_exps, il);
    }
    ggml_tensor * physical_experts = hot_physical ? hot_physical : selected_experts;
    ...
    if (gate_up_exps) {
        ...
        ggml_tensor * gate_up = build_lora_mm_id(gate_up_exps, cur, physical_experts); // [n_ff*2, n_expert_used, n_tokens]
        ...
        up = build_lora_mm_id(up_exps, cur, physical_experts); // [n_ff, n_expert_used, n_tokens]
        cb(up, "ffn_moe_up", il);
        ...
        if (gate_exps) {
            cur = build_lora_mm_id(gate_exps, cur, physical_experts); // [n_ff, n_expert_used, n_tokens]
            cb(cur, "ffn_moe_gate", il);
        } else {
            cur = up;
        }
    ...
    experts = build_lora_mm_id(down_exps, cur, physical_experts); // [n_embd, n_expert_used, n_tokens]
    ...
    if (hot_cache && n_tokens > 1) moe_out = llama_hot_union_finish(hot_cache, ctx0, moe_out, il);
