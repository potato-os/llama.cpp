...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    if (n_tokens <= LLAMA_MOE_CACHE_MAX_BATCH && !gate_up_exps && up_exps && gate_exps && down_exps &&
        ggml_is_quantized(up_exps->type) && ggml_is_quantized(gate_exps->type) &&
    ...
    if (mcache) {
        // flatten the ids so a small verify batch (n_tokens > 1) also goes through the cache
        ggml_tensor * mc_ids_flat = ggml_reshape_1d(ctx0, ggml_cont(ctx0, selected_experts), n_expert_used*n_tokens);
        mc_slot_ids = ggml_get_rows(ctx0, mcache->dev_table, mc_ids_flat); // [1, n_expert_used*n_tokens]
