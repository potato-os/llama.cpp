...
struct llama_grammar {
    ...
    // Scratch reused across llama_grammar_apply_impl calls: decoded code points
    // for every candidate live back-to-back in `candidates_arena`, with one
    // start offset per candidate in `candidates_offsets`. This avoids one heap
    // allocation per vocabulary entry on every grammar application. These are
    // caches, not grammar state -- both are cleared at the start of each apply
    // and carry no meaning between calls. Declared last so the aggregate
    // initializers elsewhere in this file keep working.
    std::vector<uint32_t> candidates_arena;
    std::vector<size_t>   candidates_offsets;
};
