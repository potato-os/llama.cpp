...
bool llama_model::load_tensors(llama_model_loader & ml) {
    if (params.hot_experts) {
        if (arch != LLM_ARCH_QWEN35MOE || hparams.n_expert != 256 || hparams.n_expert_used != 8) {
            throw std::runtime_error("hot experts require Qwen3.5 MoE with 256 experts and top-8 routing");
        }
        if (devices.size() != 1 || std::string(ggml_backend_reg_name(ggml_backend_dev_backend_reg(devices[0].dev))) != "MTL") {
            throw std::runtime_error("hot experts require a single Metal device");
        }
        if (this->n_gpu_layers() < hparams.n_layer || params.no_alloc || (params.tensor_buft_overrides && params.tensor_buft_overrides[0].pattern)) {
            throw std::runtime_error("hot experts require full GPU layer offload and default tensor placement");
        }
        hot_cache = llama_hot_create(ml.metadata, hparams.n_layer, hparams.n_expert, params.hot_experts, params.hot_expert_bias);
        ml.hot_cache = hot_cache;
    }
    ...
    for (auto & [buft, ctx_ptr] : ml.ctx_map) {
        ...
        if (hot_cache && llama_mmap::SUPPORTED && !ml.no_alloc && !use_mlock &&
                buffer_from_host_ptr_supported && is_default_buft &&
                ggml_backend_dev_type(dev) == GGML_BACKEND_DEVICE_TYPE_CPU &&
                ggml_get_first_tensor(ctx) == tok_embd && ggml_get_next_tensor(ctx, tok_embd) == nullptr) {
            const auto * weight = ml.get_weight(tok_embd->name);
            GGML_ASSERT(weight && !ml.use_mmap);
            auto & file = ml.files.at(weight->idx);
            const size_t bytes = ggml_nbytes(tok_embd);
            auto mapping = std::make_unique<llama_mmap>(file.get(), 0, true);
            mapping->unmap_fragment(0, weight->offs);
            mapping->unmap_fragment(weight->offs + bytes, file->size());
            void * data = static_cast<char *>(mapping->addr()) + weight->offs;
            auto * buf = ggml_backend_dev_buffer_from_host_ptr(dev, data, bytes, bytes);
            if (!buf) {
                throw std::runtime_error("CPU embedding mapping buffer allocation failed");
            }
            bufs.emplace_back(buf);
            ggml_backend_tensor_alloc(buf, tok_embd, data);
            ml.externally_mapped_tensors.insert(tok_embd);
            buf_map.emplace(weight->idx, buf);
            pimpl->mappings.emplace_back(std::move(mapping));
            LLAMA_LOG_INFO("%s: mapped input embeddings (%zu bytes)\n", __func__, bytes);
