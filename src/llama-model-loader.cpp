...
struct ggml_tensor * llama_model_loader::create_tensor(
    ...
    auto buft_for_tensor = [&](ggml_tensor * t_meta) -> ggml_backend_buffer_type_t {
        ...
        if (hot_cache && tn.tensor == LLM_TENSOR_TOKEN_EMBD && !(flags & TENSOR_DUPLICATED)) {
            auto * cpu = ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_CPU);
            if (!cpu) {
                throw std::runtime_error("hot experts require a CPU input buffer");
            }
            buft = ggml_backend_dev_buffer_type(cpu);
        }
    ...
    if ((hot_cache && strstr(cur->name, "_exps.weight"))) {
        if (use_mmap || cur->ne[2] != 256 || cur->ne[3] != 1) {
            throw std::runtime_error("hot cache requires --no-mmap and 256 experts");
        }
        tensor = ggml_new_tensor_3d(ctx, cur->type, cur->ne[0], cur->ne[1], llama_hot_capacity(hot_cache));
    } else {
...
bool llama_model_loader::load_all_data(
    ...
    for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
        ...
        if (externally_mapped_tensors.count(cur)) {
            GGML_ASSERT(!use_mmap && cur->data && ggml_backend_buffer_is_host(cur->buffer));
        ...
        if ((hot_cache && strstr(cur->name, "_exps.weight"))) {
            llama_hot_bind(hot_cache, cur, files.at(weight->idx)->file_id(), weight->offs, weight->tensor->nb[2]);
