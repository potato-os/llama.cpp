...
llama_context::llama_context(
    ...
    if (model.hot_cache && (params.n_seq_max > 1 || params.n_ubatch < 1 || params.n_ubatch > 512)) {
        throw std::runtime_error("hot experts require one sequence and a microbatch size from 1 to 512");
    }
    if (model.hot_cache) {
        llama_hot_attach_context(model.hot_cache);
        hot_cache_context = std::shared_ptr<void>(model.hot_cache, [](void * cache) {
            llama_hot_detach_context(static_cast<llama_hot_cache *>(cache));
        });
    }
...
ggml_status llama_context::graph_compute(
    ...
    if (status != GGML_STATUS_SUCCESS) {
        ggml_backend_sched_synchronize(sched.get());
        if (model.hot_cache) llama_hot_abort_pending(model.hot_cache);
...
size_t llama_context::state_write_data(llama_io_write_i & io) {
    ...
    if (model.hot_cache) {
        const auto data = llama_hot_save_state(model.hot_cache);
...
size_t llama_context::state_read_data(llama_io_read_i & io) {
    ...
    if (model.hot_cache) {
        uint64_t size;
        io.read_to(&size, sizeof(size));
        if (size > 1024*1024) throw std::runtime_error("invalid hot expert state size");
        std::vector<uint8_t> data(size);
        io.read_to(data.data(), data.size());
        llama_hot_load_state(model.hot_cache, data);
    }
    return io.n_bytes();
}
