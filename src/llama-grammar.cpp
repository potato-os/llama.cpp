...
// Identical to decode_utf8() below, except the code points are appended to a
// caller-supplied buffer instead of a freshly allocated vector. Used by
// llama_grammar_apply_impl, which decodes the whole vocabulary on every call:
// one allocation per candidate token (~248k for a large vocab) dominated the
// cost of applying a grammar. Same code points, same partial-utf8 result.
static llama_partial_utf8 decode_utf8_into(
    ...
    while (*pos != 0) {
        ...
        if (n_remain < 0) {
            // invalid sequence, abort
            out.resize(base);   // matches code_points.clear() on the fresh-vector path
            out.push_back(0);
...
void llama_grammar_apply_impl(const struct llama_grammar & grammar_const, llama_token_data_array * cur_p) {
    // scratch buffers below are caches, not grammar state (see llama-grammar.h)
    struct llama_grammar & grammar = const_cast<struct llama_grammar &>(grammar_const);
    ...
    // Decode every candidate into one flat arena rather than a vector per
    // candidate: this runs over the whole vocabulary on each call, so the
    // per-token allocation was the dominant cost. Offsets are recorded first
    // and turned into pointers after the loop, so arena growth cannot
    // invalidate them.
    std::vector<uint32_t> & arena   = grammar.candidates_arena;
    std::vector<size_t>   & offsets = grammar.candidates_offsets;

    arena.clear();
    offsets.clear();
    arena.reserve(2*cur_p->size);
    offsets.reserve(cur_p->size);
    ...
    for (size_t i = 0; i < cur_p->size; ++i) {
        ...
        if (grammar.vocab->is_eog(id)) {
            ...
            const llama_partial_utf8 pu = decode_utf8_into(piece, grammar.partial_utf8, arena);
            offsets.push_back(off);
            candidates_grammar.push_back({ i, nullptr, pu, id });
        }
    }

    for (size_t k = 0; k < candidates_grammar.size(); ++k) {
        candidates_grammar[k].code_points = arena.data() + offsets[k];
    }
