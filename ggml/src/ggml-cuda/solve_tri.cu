...
// Any-size forward substitution used when the batched-trsm path is not built.
// Column recurrences are independent, so each thread solves one right-hand-side
// column serially over the rows, re-reading its own earlier results through the
// cache. Correctness and coverage over speed; the warp kernel above this size
// class stays the fast path.
static __global__ void solve_tri_f32_general(const float * __restrict__ A,
    ...
    const float * A_b = A + i3*s03 + i2*s02;
    const float * B_b = B + i3*s13 + i2*s12;
    float       * X_b = X + i3*s3  + i2*s2;

    for (int row = 0; row < n; ++row) {
        float sum = B_b[row*k + col];
        for (int j = 0; j < row; ++j) {
            sum -= A_b[row*n + j] * X_b[j*k + col];
        }
        X_b[row*k + col] = sum / A_b[row*n + row];
    }
}
...
void ggml_cuda_op_solve_tri(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (n <= MAX_N_FAST && k <= MAX_K_FAST) {
        ...
        if (total_batches > 0) {
            ...
            const dim3 grid((unsigned) total_batches, (unsigned)((k + threads - 1)/threads), 1);
            solve_tri_f32_general<<<grid, threads, 0, ctx.stream()>>>(
