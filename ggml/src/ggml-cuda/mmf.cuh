...
// TODO: handle this in a consistent and simpler way after AMD MFMA support has been added
    ...
    if constexpr (has_ids) {
        ...
        // channels + tiles of ncols_dst are packed in the y dimension
        const int col_tiles = (ncols_dst_total + cols_per_block - 1) / cols_per_block;
        const int nchannels = gridDim.y / col_tiles;
        const int tile_idx  = blockIdx.y / nchannels;
        channel_dst = blockIdx.y - tile_idx * nchannels;
        col_base    = tile_idx * cols_per_block;
    }
    ...
    if constexpr (has_ids) {
        ...
        constexpr int y_stride_scale = std::is_same_v<T, float> ? 1 : 2;
        y   += (int64_t) col_base * stride_col_y * y_stride_scale;
        dst += (int64_t) col_base * stride_col_dst;
    }
...
#pragma unroll
                for (int j0 = 0; j0 < tile_B::I; ++j0) {
                    ...
                    if constexpr (!has_ids) {
                        const bool valid = j < cols_per_block && (col_base + j) < ncols_dst_total;
                        tile_xy[j0*tile_k_padded + threadIdx.x] = valid ? y[j*stride_col_y + col] : 0.0f;
                    } else {
...
#pragma unroll
                for (int j0 = 0; j0 < tile_B::I; ++j0) {
                    ...
                    if constexpr (!has_ids) {
                        const bool valid = j < cols_per_block && (col_base + j) < ncols_dst_total;
                        const float2 tmp = valid ? y2[j*stride_col_y + col] : make_float2(0.0f, 0.0f);
...
static inline void mul_mat_f_switch_ids(
    ...
    if (has_ids_data && ncols_dst > 16) {
        ...
        // channels + tiles of ncols_dst are packed in the y dimension, same
        // scheme as the ids branch above; a single tile leaves the grid as-is
        const int64_t col_tiles = (ncols_dst + cols_per_block - 1) / cols_per_block;
        dim3 block_nums_tiled = block_nums;
        block_nums_tiled.y *= col_tiles;

        mul_mat_f<T, rows_per_block, cols_per_block, nwarps, false><<<block_nums_tiled, block_dims, nbytes_shared_total, stream>>>
            (x, y, ids, dst, ncols_x, ncols_dst, nchannels_dst, stride_row, stride_col_y, stride_col_dst,
             stride_col_id, stride_row_id, channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
             sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst);
    }
...
static void mul_mat_f_switch_cols_per_block(
    ...
    // wider dst is handled by tiling the columns through the grid (both the
    // ids and the plain path); the per-launch tile stays at most 16 wide
    const int ncols_case = ncols_dst > 16 ? 16 : ncols_dst;
