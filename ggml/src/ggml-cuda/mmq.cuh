...
static constexpr __host__ __device__ tile_x_sizes mmq_get_dp4a_tile_x_sizes(ggml_type type, int mmq_y) {
    switch (type) {
        ...
        case GGML_TYPE_Q2_SYM16: return MMQ_DP4A_TXS_Q2_K;
        case GGML_TYPE_Q2_SYM32K4: return MMQ_DP4A_TXS_Q8_0_16;
        case GGML_TYPE_Q1_SYM32K: return MMQ_DP4A_TXS_Q8_0_16;
        case GGML_TYPE_Q4_SYM16K: return MMQ_DP4A_TXS_Q8_0_16;
        case GGML_TYPE_QT_MS32K4: return MMQ_DP4A_TXS_Q8_0;
...
static constexpr __host__ __device__ int mmq_get_mma_tile_x_k(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_Q2_SYM16: return MMQ_MMA_TILE_X_K_Q2_K;
        case GGML_TYPE_Q2_SYM32K4: return MMQ_MMA_TILE_X_K_Q3_K;
        case GGML_TYPE_Q1_SYM32K: return MMQ_MMA_TILE_X_K_Q3_K;
        case GGML_TYPE_Q4_SYM16K: return MMQ_MMA_TILE_X_K_Q3_K;
        case GGML_TYPE_QT_MS32K4: return MMQ_MMA_TILE_X_K_Q8_0;
...
// Q2_SYM16 shares Q2_K's tile geometry and vec_dot: Q2_K stores an arbitrary
// (dl, ml) half2 per 16-element group and evaluates dl*code - ml*sum(y).
// Ours is W = s*(code - 1.5) = s*code - 1.5*s, i.e. dl = s, ml = 1.5*s.
// s is UE4M3 (<= 4 significant bits) so it is exact in fp16, and
// 1.5*s = s + s/2 needs <= 5, also exact -- the tile reproduces the stored
// values bit-exactly, with no requantization and no change in bpw.
// Codes are stored naturally, code(j) = (qs[j/4] >> (2*(j%4))) & 3, so each
// code byte spreads into one tile int at k = 4*kqsx + l with no permutation
// (unlike Q2_K, whose k formula exists to undo its superblock interleave).
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_q2_sym16(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    ...
    half2 * x_dm = (half2 *) (x_qs + 2*MMQ_TILE_NE_K);
#else
    constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q2_SYM16, mmq_y);
    int   * x_qs = (int   *)  x_tile;
    half2 * x_dm = (half2 *) (x_qs + txs.qs);
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)

    // One thread per 16-element block; MMQ_ITER_K/QK2_SYM16 == 16 blocks per row per iteration.
    constexpr int threads_per_row = MMQ_ITER_K / QK2_SYM16;
    constexpr int nrows = ggml_cuda_get_physical_warp_size() / threads_per_row;
    const int kqsx = threadIdx.x % threads_per_row;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nrows*nwarps) {
        ...
        const block_q2_sym16 * bxi = (const block_q2_sym16 *) x + kbx0 + i*stride + kqsx;

        // Blocks are 5 B, so qs is not 4-byte aligned: read the code bytes individually.
#pragma unroll
        for (int l = 0; l < QK2_SYM16/QR2_SYM16; ++l) {
            const uint32_t qb = bxi->qs[l];

            // Spread 4 packed 2-bit codes into 4 byte lanes (as Q2_K's 0x03030303 planes).
            const int x_qs_k = (qb & 0x03) | ((qb & 0x0C) << 6) | ((qb & 0x30) << 12) | ((qb & 0xC0) << 18);

            const int k = QK2_SYM16/QR2_SYM16*kqsx + l;
        ...
        const float s = ggml_cuda_ue4m3_to_fp32(bxi->d);
        const half2 x_dm_ik = make_half2(s, 1.5f*s);
...
// Q2_SYM32K4 / Q1_SYM32K / Q4_SYM16K ride the signed-code q8_0_16 tile class
// (like IQ2_XS/Q3_K): per-16-element FLOAT scale in x_df, signed int8 codes in
// x_qs, no ml correction and no partial-sum dependence. Doubling identity
// keeps codes integral:
//   q2_sym32k4: W = 0.5*d*sc*(2c - 3)   -> codes odd +-3,    x_df = 0.5*d*sc
//   q1_sym32k: W = 0.5*d*sc*(2b - 1)   -> codes +-1,        x_df = 0.5*d*sc
//   q4_sym16k: W = 0.5*d*sc*(2c - 15)  -> codes odd +-15,   x_df = 0.5*d*sc
// Tiles are canonical: int k holds elems 4k..4k+3, scale group = k/4.
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_q2_sym32k4(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
    ...
    // One thread per 16-elem word-group; one 256-elem superblock per K iteration.
    constexpr int threads_per_row = MMQ_ITER_K / 16;
    constexpr int nrows = warp_size / threads_per_row;
    const int kqsx = threadIdx.x % threads_per_row;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nrows*nwarps) {
        ...
        const block_q2_sym32k4 * bxi = (const block_q2_sym32k4 *) x + kbx0 + i*stride;

        const int sb = kqsx / 2;            // sub-block of 32
        const int w  = get_int_b2(bxi->qs, kqsx);

        const int p0 = (w >> 0) & 0x03030303;
        const int p1 = (w >> 2) & 0x03030303;
        const int p2 = (w >> 4) & 0x03030303;
        const int p3 = (w >> 6) & 0x03030303;
        const int t01a = __byte_perm(p0, p1, 0x5140);
        const int t23a = __byte_perm(p2, p3, 0x5140);
        const int t01b = __byte_perm(p0, p1, 0x7362);
        const int t23b = __byte_perm(p2, p3, 0x7362);
        const int v[4] = {
            __byte_perm(t01a, t23a, 0x5410),
            __byte_perm(t01a, t23a, 0x7632),
            __byte_perm(t01b, t23b, 0x5410),
            __byte_perm(t01b, t23b, 0x7632),
        };

#pragma unroll
        for (int l = 0; l < 4; ++l) {
            const int x_qs_k = __vsub4(v[l] + v[l], 0x03030303);
            const int k = 4*kqsx + l;
        ...
        const uint8_t sc = (sb & 1) ? (bxi->scales[sb >> 1] >> 4) : (bxi->scales[sb >> 1] & 0x0F);
        const float df = 0.5f * __half2float(bxi->d) * (float) sc;

#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
        x_df[i*MMQ_MMA_TILE_X_K_Q3_K + kqsx] = df;
#else
        x_df[i*(2*MMQ_TILE_NE_K*2/QI8_0) + i/(QI8_0/4) + kqsx] = df;
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
    }
}

template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_q1_sym32k(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
    ...
    // One thread per 16-elem group; one 256-elem superblock per K iteration.
    constexpr int threads_per_row = MMQ_ITER_K / 16;
    constexpr int nrows = warp_size / threads_per_row;
    const int kqsx = threadIdx.x % threads_per_row;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nrows*nwarps) {
        ...
        const block_q1_sym32k * bxi = (const block_q1_sym32k *) x + kbx0 + i*stride;

        const int sb = kqsx / 2;            // sub-block of 32
        const int h  = kqsx % 2;            // 16-elem half within it
        const int bits16 = bxi->qs[4*sb + 2*h] | (bxi->qs[4*sb + 2*h + 1] << 8);

#pragma unroll
        for (int l = 0; l < 4; ++l) {
            const int n = (bits16 >> (4*l)) & 0x0F;
            const int m = (n * 0x00204081) & 0x01010101;      // bit i -> byte lane i
            const int x_qs_k = __vsub4(m << 1, 0x01010101);   // bytes: +-1
            const int k = 4*kqsx + l;
...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_q4_sym16k(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
    ...
    // One thread per 16-elem sub-block; two 128-elem blocks per K iteration.
    constexpr int threads_per_row = MMQ_ITER_K / QK4_SYM16K_SUB;
    constexpr int nrows = warp_size / threads_per_row;
    const int kqsx = threadIdx.x % threads_per_row;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nrows*nwarps) {
        ...
        // kbx0 is in 128-elem block units (blocks_per_iter = MMQ_ITER_K/qk = 2)
        const block_q4_sym16k * bxi = (const block_q4_sym16k *) x
            + kbx0 + i*stride + kqsx/QK4_SYM16K_SUPER;
        const int sb = kqsx % QK4_SYM16K_SUPER;

        // PAIRWISE nibbles: byte j of the sub-block holds elems 2j (low), 2j+1 (high)
        const uint8_t * qs = bxi->qs + sb*(QK4_SYM16K_SUB/2);

#pragma unroll
        for (int l = 0; l < 4; ++l) {
            const int b0 = qs[2*l + 0];
            const int b1 = qs[2*l + 1];
            const int c  = (b0 & 0x0F) | ((b0 >> 4) << 8) | ((b1 & 0x0F) << 16) | ((b1 >> 4) << 24);
            const int x_qs_k = __vsub4(c << 1, 0x0F0F0F0F);   // bytes: 2c - 15, odd, +-15
            const int k = 4*kqsx + l;
        ...
        const float df = 0.5f * __half2float(bxi->d) * (float) bxi->scales[sb];
...
// QT_MS32K4 uses Q8_0 tiles: signed codes -1/0/+1 and one float scale per 32 values.
// Even lanes write the shared scale; both halves retain their own code stores. One thread per
// 16-elem group, 16 threads per row: the running sign start is an exclusive prefix popcount of the
// row's 16-bit mask words, done as a 16-lane segmented warp scan.
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_qt_ms32k4(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
    ...
    constexpr int threads_per_row = MMQ_ITER_K / 16;
    static_assert(threads_per_row == 16, "QT_MS32K4 tile loader assumes 16 threads per 256-elem row");
    constexpr int nrows = warp_size / threads_per_row;
    const int kqsx = threadIdx.x % threads_per_row;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nrows*nwarps) {
        ...
        const block_qt_ms32k4 * bxi = (const block_qt_ms32k4 *) x + kbx0 + i*stride;

        const int bits16 = ((const uint16_t *) bxi->mask)[kqsx];      // mask of elems 16*kqsx .. +15
        const int own    = __popc(bits16);
        int incl = own;
#pragma unroll
        for (int o = 1; o < threads_per_row; o <<= 1) {
            const int t = __shfl_up_sync(0xFFFFFFFF, incl, o, threads_per_row);
            if (kqsx >= o) {
                incl += t;
            }
        }
        const int start = incl - own;

        const uint16_t * s16 = (const uint16_t *) bxi->signs;
        const int si = start >> 4;
        uint32_t sw = ((uint32_t) s16[min(si, 7)] | ((uint32_t) s16[min(si + 1, 7)] << 16)) >> (start & 15);

#pragma unroll
        for (int l = 0; l < 4; ++l) {
            const int x_qs_k = qt_ms32k4_lanes((bits16 >> (4*l)) & 0x0F, sw);
            const int k = 4*kqsx + l;

#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
            x_qs[i*MMQ_MMA_TILE_X_K_Q8_0 + k] = x_qs_k;
#else
            x_qs[i*(2*MMQ_TILE_NE_K + 1) + k] = x_qs_k;
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
        }
        ...
        const int sb = kqsx / 2;
        const uint8_t sc = (sb & 1) ? (bxi->scales[sb >> 1] >> 4) : (bxi->scales[sb >> 1] & 0x0F);
        const float df = __half2float(bxi->d) * (float) sc;

#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
        if ((kqsx & 1) == 0) {
            x_df[i*MMQ_MMA_TILE_X_K_Q8_0 + sb] = df;
        }
#else
        if ((kqsx & 1) == 0) {
            x_df[i*(2*MMQ_TILE_NE_K/QI8_0) + i/(QI8_0/2) + sb] = df;
        }
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
    }
}
...
template <int mmq_x, int mmq_y, bool need_check>
struct mmq_type_traits<mmq_x, mmq_y, need_check, GGML_TYPE_Q2_SYM16> {
    static constexpr int              vdr          = VDR_Q2_K_Q8_1_MMQ;
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_q2_sym16<mmq_y, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q2_K_q8_1_mma<mmq_x, mmq_y>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_q2_K_q8_1_dp4a<mmq_x, mmq_y>;
};

template <int mmq_x, int mmq_y, bool need_check>
struct mmq_type_traits<mmq_x, mmq_y, need_check, GGML_TYPE_Q2_SYM32K4> {
    static constexpr int              vdr          = VDR_IQ2_XS_Q8_1_MMQ;
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_q2_sym32k4<mmq_y, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q8_0_16_q8_1_mma<mmq_x, mmq_y>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_q8_0_16_q8_1_dp4a<mmq_x, mmq_y>;
};

template <int mmq_x, int mmq_y, bool need_check>
struct mmq_type_traits<mmq_x, mmq_y, need_check, GGML_TYPE_Q1_SYM32K> {
    static constexpr int              vdr          = VDR_IQ2_XS_Q8_1_MMQ;
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_q1_sym32k<mmq_y, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q8_0_16_q8_1_mma<mmq_x, mmq_y>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_q8_0_16_q8_1_dp4a<mmq_x, mmq_y>;
};

template <int mmq_x, int mmq_y, bool need_check>
struct mmq_type_traits<mmq_x, mmq_y, need_check, GGML_TYPE_QT_MS32K4> {
    static constexpr int              vdr          = VDR_Q8_0_Q8_1_MMQ;
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_qt_ms32k4<mmq_y, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q8_0_q8_1_mma<mmq_x, mmq_y, MMQ_Q8_1_DS_LAYOUT_D4>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_q8_0_q8_1_dp4a<mmq_x, mmq_y>;
};

template <int mmq_x, int mmq_y, bool need_check>
struct mmq_type_traits<mmq_x, mmq_y, need_check, GGML_TYPE_Q4_SYM16K> {
    static constexpr int              vdr          = VDR_IQ2_XS_Q8_1_MMQ;
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_q4_sym16k<mmq_y, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q8_0_16_q8_1_mma<mmq_x, mmq_y>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_q8_0_16_q8_1_dp4a<mmq_x, mmq_y>;
};
...
void mul_mat_q_case(ggml_backend_cuda_context & ctx, const mmq_args & args, cudaStream_t stream) {
    ...
    // Cap tile width for sparsely populated experts to avoid padded MMA work.
    // Dense per-expert batches retain wider tiles.
    if (args.ids_dst != nullptr && args.nchannels_x > 0) {
        const int64_t cols_per_expert = args.ncols_dst / args.nchannels_x;
        if (cols_per_expert < 4) {
    ...
    // For the ids (MoE) path, tile occupancy depends on columns per expert.
    // GGML_MMQ_X_ID overrides the tile width for this path.
    if (args.ids_dst != nullptr && getenv("GGML_MMQ_X_ID") != nullptr) {
        const int forced = atoi(getenv("GGML_MMQ_X_ID"));
        switch (forced) {
            case   8: launch_mul_mat_q<type,   8>(ctx, args, stream); return;
            case  16: launch_mul_mat_q<type,  16>(ctx, args, stream); return;
            case  32: launch_mul_mat_q<type,  32>(ctx, args, stream); return;
            case  64: launch_mul_mat_q<type,  64>(ctx, args, stream); return;
            case 128: launch_mul_mat_q<type, 128>(ctx, args, stream); return;
            default: break;
        }
    }
...
extern DECL_MMQ_CASE(GGML_TYPE_Q2_SYM16);
extern DECL_MMQ_CASE(GGML_TYPE_Q1_SYM32K);
extern DECL_MMQ_CASE(GGML_TYPE_Q4_SYM16K);
extern DECL_MMQ_CASE(GGML_TYPE_QT_MS32K4);
