...
#define VDR_Q4_SYM16_Q8_1_MMVQ 2

static __device__ __forceinline__ float vec_dot_q4_sym16_q8_1(
    const void * __restrict__ vbq, const block_q8_1 * __restrict__ bq8_1, const int & kbx, const int & iqs) {

    // W = d_g*(code - 7.5), one UE4M3 scale per 16-element group, two groups
    // per 32-element block. Each call (iqs = 0 or 2) covers exactly one
    // group. Same unsigned-nibble core as Q4_1; the -7.5 offset uses an
    // integer per-group activation sum (exact), not q8_1's stored fp16 sum.
    const block_q4_sym16 * bq4 = (const block_q4_sym16 *) vbq + kbx;
    const int g = iqs >> 1;
...
#pragma unroll
    for (int i = 0; i < 2; ++i) {
        ...
        const int u_lo = get_int_b4(bq8_1->qs, 4*g + i);
        const int u_hi = get_int_b4(bq8_1->qs, 4*g + i + 2);

        sumi = ggml_cuda_dp4a((v >> 0) & 0x0F0F0F0F, u_lo, sumi);
        sumi = ggml_cuda_dp4a((v >> 4) & 0x0F0F0F0F, u_hi, sumi);
        ysum = ggml_cuda_dp4a(0x01010101, u_lo, ysum);
        ysum = ggml_cuda_dp4a(0x01010101, u_hi, ysum);
    }

    const float d  = ggml_cuda_ue4m3_to_fp32(bq4->d[g]);
    const float d8 = __low2float(bq8_1->ds);

    return d * d8 * (sumi - 7.5f * ysum);
}

#define VDR_Q2_SYM32K4_Q8_1_MMVQ 2

static __device__ __forceinline__ float vec_dot_q2_sym32k4_q8_1(
    const void * __restrict__ vbq, const block_q8_1 * __restrict__ bq8_1, const int & kbx, const int & iqs) {

    // W = d*sc_s*(code - 1.5), 4-bit unsigned nibble sub-scales; sub-block of
    // 32 aligns exactly with one q8_1 block. One call (iqs = 0,2,..,14;
    // vdr = 2) covers one sub-block = two 32-bit code words. Codes are
    // sequential LSB-first (byte b = elems 4b..4b+3), so each word's four
    // 2-bit planes are interleaved back to sequential byte order with
    // __byte_perm, then the doubling identity (2c - 3, bytes in [-3,3]) keeps
    // the -1.5 offset integer-exact -- no activation-sum correction needed.
    const block_q2_sym32k4 * bq2 = (const block_q2_sym32k4 *) vbq + kbx;
    const int s = iqs >> 1;
...
#pragma unroll
    for (int t = 0; t < 2; ++t) {
        const int w  = get_int_b2(bq2->qs, iqs + t);
        ...
        const int v0 = __byte_perm(t01a, t23a, 0x5410);
        const int v1 = __byte_perm(t01a, t23a, 0x7632);
        const int v2 = __byte_perm(t01b, t23b, 0x5410);
        const int v3 = __byte_perm(t01b, t23b, 0x7632);

        const int base = 4*t;
        sumi = ggml_cuda_dp4a(__vsub4(v0 + v0, 0x03030303), get_int_b4(b8->qs, base + 0), sumi);
        sumi = ggml_cuda_dp4a(__vsub4(v1 + v1, 0x03030303), get_int_b4(b8->qs, base + 1), sumi);
        sumi = ggml_cuda_dp4a(__vsub4(v2 + v2, 0x03030303), get_int_b4(b8->qs, base + 2), sumi);
        sumi = ggml_cuda_dp4a(__vsub4(v3 + v3, 0x03030303), get_int_b4(b8->qs, base + 3), sumi);
    }

    const uint8_t sc = (s & 1) ? (bq2->scales[s >> 1] >> 4) : (bq2->scales[s >> 1] & 0x0F);
    const float ds = __half2float(bq2->d) * (float) sc;
    const float d8 = __low2float(b8->ds);

    return 0.5f * ds * d8 * (float) sumi;
}

#define VDR_Q1_SYM32K_Q8_1_MMVQ 1

static __device__ __forceinline__ float vec_dot_q1_sym32k_q8_1(
    const void * __restrict__ vbq, const block_q8_1 * __restrict__ bq8_1, const int & kbx, const int & iqs) {

    // W = 0.5*d*sc_s*(2b - 1), b the LSB-first bit stream; sub-block of 32
    // aligns exactly with one q8_1 block. One call (iqs = word index 0..7)
    // covers one sub-block: expand each bit nibble to a +-1 byte mask
    // ((n*0x00204081)&0x01010101 spreads bit i of n to byte i) and dp4a
    // against the activations -- integer-exact, no activation sum needed.
    const block_q1_sym32k * bq1 = (const block_q1_sym32k *) vbq + kbx;
    const int s = iqs;

    const int q = get_int_b2(bq1->qs, s);
    const block_q8_1 * b8 = bq8_1 + s;

    int sumi = 0;
#pragma unroll
    for (int t = 0; t < 8; ++t) {
        const int u = get_int_b4(b8->qs, t);
        const int n = (q >> (4*t)) & 0x0F;
        const int m = __vsub4(((n * 0x00204081) & 0x01010101) << 1, 0x01010101); // bytes: 2b-1
        sumi = ggml_cuda_dp4a(m, u, sumi);
    }

    const uint8_t sc = (s & 1) ? (bq1->scales[s >> 1] >> 4) : (bq1->scales[s >> 1] & 0x0F);
    const float  d  = __half2float(bq1->d);
    const float  d8 = __low2float(b8->ds);
...
static __device__ __forceinline__ float vec_dot_q4_sym16k_q8_1(
    ...
    // W = d*scales[s]*(code - 7.5), int8 sub-scales, sub-block of 16 with
    // PAIRWISE nibbles (byte j = elems 2j, 2j+1). One call (iqs = 0,2,..,14;
    // vdr = 2) covers one sub-block = half a q8_1 block. Interleave the
    // nibble planes back to sequential order with __byte_perm, then the
    // same integer-exact -7.5 handling as the flat q4_sym16 above.
    const block_q4_sym16k * bq4 = (const block_q4_sym16k *) vbq + kbx;
    const int s = iqs >> 1;

    const block_q8_1 * b8 = bq8_1 + (s >> 1);
    const int yoff = 4 * (s & 1);

    int sumi = 0;
    int ysum = 0;
#pragma unroll
    for (int i = 0; i < 2; ++i) {
        const int w  = get_int_b2(bq4->qs, iqs + i);
        const int lo = (w >> 0) & 0x0F0F0F0F;               // elems 0,2,4,6 of the 8
        const int hi = (w >> 4) & 0x0F0F0F0F;               // elems 1,3,5,7
        const int v0 = __byte_perm(lo, hi, 0x5140);         // elems 0,1,2,3 sequential
        const int v1 = __byte_perm(lo, hi, 0x7362);         // elems 4,5,6,7 sequential

        const int u0 = get_int_b4(b8->qs, yoff + 2*i + 0);
        const int u1 = get_int_b4(b8->qs, yoff + 2*i + 1);

        sumi = ggml_cuda_dp4a(v0, u0, sumi);
        sumi = ggml_cuda_dp4a(v1, u1, sumi);
        ysum = ggml_cuda_dp4a(0x01010101, u0, ysum);
        ysum = ggml_cuda_dp4a(0x01010101, u1, ysum);
    }

    const float ds = __half2float(bq4->d) * (float) bq4->scales[s];
    const float d8 = __low2float(b8->ds);

    return ds * d8 * (sumi - 7.5f * ysum);
}
