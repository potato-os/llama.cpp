...
static __global__ void quantize_q8_1(
        ...
        const int64_t ne0, const uint32_t ne1, const uint3 ne2, const uint32_t ne23) {
    ...
    for (uint32_t i23 = blockIdx.z; i23 < ne23; i23 += gridDim.z) {
        const int64_t i3 = fastdiv(i23, ne2);
        const int64_t i2 = i23 - i3*ne2.z;

        for (uint32_t i1 = blockIdx.y; i1 < ne1; i1 += gridDim.y) {
...
void quantize_row_q8_1_cuda(
    ...
    GGML_ASSERT(block_num_x <= UINT_MAX);
    GGML_ASSERT(ne1 <= UINT_MAX);
    GGML_ASSERT(ne2*ne3 <= UINT_MAX);
    const dim3 num_blocks(block_num_x, MIN(ne1, UINT16_MAX), MIN(ne2*ne3, UINT16_MAX));
    const dim3 block_size(CUDA_QUANTIZE_BLOCK_SIZE, 1, 1);
    quantize_q8_1<<<num_blocks, block_size, 0, stream>>>(
        x, vy, ne00, s01, s02, s03, ne0, ne1, ne2_fastdiv, ne2*ne3);
