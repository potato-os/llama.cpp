...
const int CUDA_CPY_MAX_GRID_Z = USHRT_MAX;
...
static __global__ void cpy_scalar_transpose(const char * cx, char * cdst, const int64_t ne,
                               const int64_t ne00, const int64_t ne01, const int64_t ne02, const int64_t nb00, const int64_t nb01, const int64_t nb02,
                               const int64_t nb03, const int64_t ne10, const int64_t ne11, const int64_t ne12, const int64_t nb10, const int64_t nb11,
                               const int64_t nb12, const int64_t nb13, const int64_t imat_block_offset) {
...
#pragma unroll
    for (int i = 0; i < CUDA_CPY_BLOCK_NM; ++i) {
        ...
        const int64_t imat = (imat_block_offset + blockIdx.z) * CUDA_CPY_BLOCK_NM + i;
        if (imat >= nmat)
            break;
...
static void ggml_cpy_scalar_cuda(
    ...
    if (transposed) {
        ...
        for (int64_t imat_block_offset = 0; imat_block_offset < grid_z; imat_block_offset += CUDA_CPY_MAX_GRID_Z) {
            const unsigned int grid_z_chunk = (unsigned int) MIN(grid_z - imat_block_offset, (int64_t) CUDA_CPY_MAX_GRID_Z);
            dim3 dimGrid(grid_x, grid_y, grid_z_chunk);
            cpy_scalar_transpose<dst_t><<<dimGrid, dimBlock, 0, stream>>>
                (cx, cdst, ne, ne00n, ne01n, ne02n, nb00, nb01, nb02, nb03, ne10, ne11, ne12, nb10, nb11, nb12, nb13, imat_block_offset);
        }
    } else {
        const int64_t num_blocks = (ne + CUDA_CPY_BLOCK_SIZE - 1) / CUDA_CPY_BLOCK_SIZE;
        GGML_ASSERT(num_blocks < UINT_MAX);
