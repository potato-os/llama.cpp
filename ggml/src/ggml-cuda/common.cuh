...
template<>
struct ggml_cuda_type_traits<GGML_TYPE_Q4_SYM16> {
    static constexpr int qk = QK4_SYM16;
    static constexpr int qr = 2;
    static constexpr int qi = 4;
};

template<>
struct ggml_cuda_type_traits<GGML_TYPE_Q2_SYM32K4> {
    static constexpr int qk = QK2_SYM32K;
    static constexpr int qr = QR2_SYM32K;
    static constexpr int qi = QI2_SYM32K;
};

template<>
struct ggml_cuda_type_traits<GGML_TYPE_Q1_SYM32K> {
    static constexpr int qk = QK1_SYM32K;
    static constexpr int qr = QR1_SYM32K;
    static constexpr int qi = QI1_SYM32K;
};

template<>
struct ggml_cuda_type_traits<GGML_TYPE_Q4_SYM16K> {
    static constexpr int qk = QK4_SYM16K;
    static constexpr int qr = QR4_SYM16K;
...
template<>
struct ggml_cuda_type_traits<GGML_TYPE_Q2_SYM16> {
    static constexpr int qk = QK2_SYM16;
    static constexpr int qr = QR2_SYM16;
    static constexpr int qi = QI2_SYM16;
};
...
struct ggml_backend_cuda_context {
    ...
    // Per-eval reuse cache for the MoE mul_mat_id setup: within one graph
    // eval the gate and up projections of a layer share the same ids tensor
    // AND the same src1 layout, so the sorted-ids helper and the activation
    // quantization run once for the pair. The down projection has different
    // src1 strides (si1/sis1 feed the helper) and never matches the key.
    struct mmq_id_cache_t {
        int64_t      eval_no     = -1;
        const void * ids_tensor  = nullptr;
        ...
        int64_t      ne02        = 0;
        size_t       q8_bytes    = 0;
        std::optional<ggml_cuda_pool_alloc<int32_t>> ids_src1;
        std::optional<ggml_cuda_pool_alloc<int32_t>> ids_dst;
        std::optional<ggml_cuda_pool_alloc<int32_t>> expert_bounds;
        std::optional<ggml_cuda_pool_alloc<char>>    src1_q8_1;
        void reset() {
            src1_q8_1.reset(); expert_bounds.reset(); ids_dst.reset(); ids_src1.reset();
            ids_tensor = nullptr; src1_tensor = nullptr; si1 = -1; sis1 = -1; q8_bytes = 0;
        }
    } mmq_id_cache;
    int64_t eval_no = 0;
