...
// TQ2_0 (Maple): 256 elements per block, one f16 scale, crumbs stored as bit planes -- bytes [0,32) hold elements
// 0..31 at shift 0, 32..63 at shift 2, 64..95 at shift 4, 96..127 at shift 6, then bytes [32,64) the same for
// elements 128..255 (see dequantize_row_tq2_0). Each thread takes one 32-element chunk = one (group, plane) pair and
// writes it as 8 int8x4 words, so the tile ends up in the Q8_0 layout and the Q8_0 dot products can be reused.
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_load_tiles_tq2_0(
    ...
    constexpr int blocks_per_iter           = MMQ_ITER_K / QK_K;   // 1
    constexpr int chunks_per_block          = QK_K / QK8_1;        // 8 chunks of 32 elements
    constexpr int threads_per_row           = blocks_per_iter * chunks_per_block;
    constexpr int nrows                     = warp_size / threads_per_row;
    constexpr int scale_entries_per_block   = chunks_per_block;
    constexpr int scale_entries_per_row     = blocks_per_iter * scale_entries_per_block;

    const int txi   = threadIdx.x % threads_per_row;
    const int kbx   = txi / chunks_per_block;
    const int kqsx  = txi % chunks_per_block;
    const int group = (kqsx / 4) * 32;   // byte offset of this chunk's 32-byte group
    const int shift = 2 * (kqsx % 4);    // bit plane inside those bytes
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nrows*nwarps) {
        ...
        const block_tq2_0 * bxi = (const block_tq2_0 *) x + kbx0 + i*stride + kbx;

        const int dst_offset = kbx*(scale_entries_per_block*QI8_0) + kqsx*QI8_0;

#pragma unroll
        for (int j = 0; j < QI8_0; ++j) {
            const int q = get_int_b2(bxi->qs + group, j);
            const int v = tq2_0_crumbs_to_int8(q, shift);
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nwarps) {
        ...
        const block_tq2_0 * bxi = (const block_tq2_0 *) x + kbx0 + i*stride + scale_block;
