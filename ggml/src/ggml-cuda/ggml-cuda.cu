...
#include "ggml-cuda/gemm-fallback.cuh"
...
#include "ggml-cuda/mmvq2sym16.cuh"
...
struct ggml_cuda_pool_vmm : public ggml_cuda_pool {
    ...
    void * alloc(size_t size, size_t * actual_size) override {
        ...
        if (size > avail) {
            ...
            // set access for the owning device and any devices that can access it via P2P
            const int device_count = ggml_backend_cuda_get_device_count();
            std::vector<CUmemAccessDesc> access;
            access.reserve(device_count);
            for (int id = 0; id < device_count; ++id) {
                int can_access = id == device;
                if (id != device) {
                    CUDA_CHECK(cudaDeviceCanAccessPeer(&can_access, id, device));
                }
                if (!can_access) {
                    continue;
                }
            ...
            CU_CHECK(cuMemSetAccess((CUdeviceptr)((char *)(pool_addr) + pool_size), reserve_size, access.data(), access.size()));
...
ggml_backend_cuda_context::~ggml_backend_cuda_context() {
    ...
    // pool-backed cache allocations must die before the pools do (the cache
    // member is declared before `pools`, so default destruction order would
    // run it after the pools are gone)
    mmq_id_cache.reset();
...
#ifndef GGML_CUDA_NO_CUBLAS
        if (cublas_handles[i] != nullptr) {
            CUBLAS_CHECK(cublasDestroy(cublas_handles[i]));
        }
#endif // GGML_CUDA_NO_CUBLAS
    }
}
...
static bool ggml_cuda_should_fuse_mul_mat_vec_q(const ggml_tensor * tensor) {
    ...
    bool use_mul_mat_vec_q = ggml_is_quantized(src0->type) && src0->type != GGML_TYPE_Q2_SYM16 && !bad_padding_clear && src1->type == GGML_TYPE_F32 &&
...
static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    bool use_mul_mat_f     = !ggml_is_quantized(src0->type)
        && src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32;
    bool use_mul_mat_vec_q = ggml_is_quantized(src0->type) && src0->type != GGML_TYPE_Q2_SYM16 && !bad_padding_clear
        && src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32
...
#ifdef GGML_CUDA_NO_CUBLAS
    use_batched_cublas_f16  = false;
    use_batched_cublas_bf16 = false;
    use_batched_cublas_f32  = false;
#endif // GGML_CUDA_NO_CUBLAS

    if (!split && src0->type == GGML_TYPE_Q2_SYM16 && ggml_cuda_should_use_mmvq2sym16(src0->ne, src1->ne[1])) {
        ggml_cuda_mul_mat_vec_q2sym16(ctx, src0, src1, nullptr, dst);
...
#else
        GGML_ABORT("unreachable: batched cuBLAS flags are cleared in this build");
#endif // GGML_CUDA_NO_CUBLAS
...
#else
        // Builds without cuBLAS route every remaining shape through the
        // generic tiled kernel. Quantized src0 that the quantized kernels
        // rejected is dequantized to f32 first; float types go in directly.
        GGML_ASSERT(!split && "mul_mat on split buffers requires cuBLAS");
        if (ggml_is_quantized(src0->type)) {
            const int64_t n_elems = ggml_nelements(src0);
            ggml_cuda_pool_alloc<float> src0_f32(ctx.pool(), n_elems);
            const to_fp32_cuda_t to_fp32 = ggml_get_to_fp32_cuda(src0->type);
            GGML_ASSERT(to_fp32 != nullptr);
            to_fp32(src0->data, src0_f32.get(), n_elems, ctx.stream());
            // The dequantized buffer preserves the source's storage order, so
            // the f32 view keeps the source's (possibly permuted) strides,
            // rescaled from quant-block bytes to f32 bytes.
            const size_t  ts = ggml_type_size(src0->type);
            const int64_t bs = ggml_blck_size(src0->type);
            ggml_tensor src0_view = *src0;
            src0_view.type  = GGML_TYPE_F32;
            src0_view.data  = src0_f32.get();
            src0_view.nb[0] = sizeof(float);
            for (int i = 1; i < GGML_MAX_DIMS; ++i) {
                src0_view.nb[i] = src0->nb[i]/ts*bs*sizeof(float);
            }
            ggml_cuda_mul_mat_gemm_fallback(ctx, &src0_view, src1, dst);
        } else {
            ggml_cuda_mul_mat_gemm_fallback(ctx, src0, src1, dst);
        }
#endif // GGML_CUDA_NO_CUBLAS
    }
}
...
static void ggml_cuda_mul_mat_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
        if (src0->type == GGML_TYPE_Q2_SYM16 && ne2 == 1) {
            ggml_cuda_mul_mat_vec_q2sym16(ctx, src0, src1, ids, dst);
            return;
        }
        static_assert(MMVQ_MAX_BATCH_SIZE == MMVF_MAX_BATCH_SIZE);
        // The mmvq-id path uses one channel per (token, expert) pair.
        // GGML_MMVQ_ID_MAX overrides its batch ceiling for supported types.
        static const int mmvq_id_max_env = getenv("GGML_MMVQ_ID_MAX") ? atoi(getenv("GGML_MMVQ_ID_MAX")) : 0;
        if (mmvq_id_max_env > 0 && ggml_is_quantized(src0->type) && ne2 <= mmvq_id_max_env &&
            get_mmvq_mmid_max_batch(src0->type, cc) > 0) { // excludes types without an mmvq vec_dot
...
static bool ggml_cuda_graph_check_compability(ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (node->op == GGML_OP_MUL_MAT_ID) {
            ...
            // Q2_SYM16 at ne2==1 dispatches to the fused mmvq2sym16 kernel,
            // which reads ids on-device with no stream sync -- graph-safe
            // despite its mmvq_mmid_max of 0 (that 0 only guards the
            // mmvq-template path, which has no kernel for this type).
            const bool q2sym16_fused = node->src[0]->type == GGML_TYPE_Q2_SYM16 && node->ne[2] == 1;
            // The MMQ id path also resolves ids entirely on-device (see
            // ggml_cuda_launch_mm_ids_helper), so it is graph-safe at any
            // batch size; only the sorted-expert fallback below it copies
            // ids to the host and forces a stream sync, preventing graph
            // capture and adding per-launch overhead.
            static const bool graph_mmid = getenv("GGML_CUDA_GRAPH_MMID") == nullptr ||
                                           atoi(getenv("GGML_CUDA_GRAPH_MMID")) != 0;
            const bool mmq_id_safe = graph_mmid && ggml_is_quantized(node->src[0]->type) &&
                ggml_cuda_should_use_mmq(node->src[0]->type, cc, node->src[1]->ne[2], node->src[0]->ne[2]);
            if (!q2sym16_fused && !mmq_id_safe && (!ggml_is_quantized(node->src[0]->type) || node->ne[2] > mmvq_mmid_max)) {
...
// Opt-in per-op wall-clock profiling: GGML_CUDA_OP_TIME=1 (forces the
// non-graph path so per-node events are legal; adds one stream sync per
// graph eval). Summary prints on process exit.
struct ggml_cuda_op_time_acc {
    std::map<std::string, std::pair<double, int64_t>> acc; // key -> (ms, count)
    std::vector<std::pair<cudaEvent_t, cudaEvent_t>> pool;
    std::vector<std::pair<std::string, std::pair<cudaEvent_t, cudaEvent_t>>> inflight;
    static bool enabled() {
        static const bool e = getenv("GGML_CUDA_OP_TIME") != nullptr;
        return e;
    }
    std::pair<cudaEvent_t, cudaEvent_t> get() {
        if (!pool.empty()) { auto p = pool.back(); pool.pop_back(); return p; }
        cudaEvent_t a, b;
    ...
    void flush(cudaStream_t stream) {
        ...
        for (auto & e : inflight) {
            float ms = 0.f;
            cudaEventElapsedTime(&ms, e.second.first, e.second.second);
            auto & slot = acc[e.first];
            slot.first += ms; slot.second++;
            pool.push_back(e.second);
        }
        inflight.clear();
    }
    ~ggml_cuda_op_time_acc() {
        if (acc.empty()) return;
        double total = 0;
        for (auto & kv : acc) total += kv.second.first;
        fprintf(stderr, "\n== GGML_CUDA_OP_TIME summary (total %.1f ms) ==\n", total);
        std::vector<std::pair<double, std::string>> rows;
        for (auto & kv : acc) rows.push_back({kv.second.first, kv.first});
        std::sort(rows.rbegin(), rows.rend());
        for (auto & r : rows) {
            fprintf(stderr, "%8.1f ms  %5.1f%%  n=%-8" PRId64 " %s\n",
                    r.first, 100.0*r.first/total, acc[r.second].second, r.second.c_str());
        }
    }
};
static ggml_cuda_op_time_acc g_cuda_op_time;

static std::string ggml_cuda_op_time_key(const ggml_tensor * node) {
    std::string k = ggml_op_name(node->op);
    if (node->op == GGML_OP_MUL_MAT || node->op == GGML_OP_MUL_MAT_ID) {
        k += std::string("(") + ggml_type_name(node->src[0]->type) + ")";
        char buf[192];
        // src0 name identifies the weight (strip the "blk.N." prefix so layers group)
        const char * w = node->src[0]->name;
        const char * dot = strchr(w, '.');
        const char * w2 = (w[0] == 'b' && dot) ? strchr(dot + 1, '.') : nullptr;
        snprintf(buf, sizeof(buf), "[%d,%d] w=%s dst[%d,%d,%d]",
                 (int) node->src[0]->ne[0], (int) node->src[0]->ne[1],
                 w2 ? w2 + 1 : w,
                 (int) node->ne[0], (int) node->ne[1], (int) node->ne[2]);
        k += buf;
        if (node->src[0]->type == GGML_TYPE_F32) {
            const ggml_tensor * y = node->src[1];
            snprintf(buf, sizeof(buf), " y[%d,%d,%d]%s%s", (int) y->ne[0], (int) y->ne[1], (int) y->ne[2],
                     ggml_is_contiguous(y) ? "" : " NONCONT",
                     ggml_is_contiguous(node) ? "" : " DSTNC");
            k += buf;
        }
    ...
    while (!graph_evaluated_or_captured) {
        ...
        if (!use_cuda_graph || cuda_graph_update_required) {
            ...
            for (int i = 0; i < cgraph->n_nodes; i++) {
                ...
                const bool op_time = ggml_cuda_op_time_acc::enabled();
                std::pair<cudaEvent_t, cudaEvent_t> op_time_ev;
                if (op_time) {
                    op_time_ev = g_cuda_op_time.get();
                    CUDA_CHECK(cudaEventRecord(op_time_ev.first, cuda_ctx->stream()));
                }
                ...
                GGML_ASSERT(ok);
                if (op_time) {
                    CUDA_CHECK(cudaEventRecord(op_time_ev.second, cuda_ctx->stream()));
                    g_cuda_op_time.inflight.push_back({ggml_cuda_op_time_key(node), op_time_ev});
                }
            ...
            if (ggml_cuda_op_time_acc::enabled()) {
                g_cuda_op_time.flush(cuda_ctx->stream());
            }
        }
...
#ifdef USE_CUDA_GRAPH
    if (ggml_cuda_op_time_acc::enabled()) {
        // per-op timing requires the non-graph path
