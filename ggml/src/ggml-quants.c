...
void dequantize_row_q4_sym16f(const block_q4_sym16f * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % 16 == 0);
    for (int64_t i = 0; i < k/16; ++i) {
        const float d = GGML_FP16_TO_FP32(x[i].d);
        for (int j = 0; j < 8; ++j) {
            y[i*16+2*j] = ((x[i].qs[j] & 15)-7.5f)*d;
            y[i*16+2*j+1] = ((x[i].qs[j] >> 4)-7.5f)*d;
        }
    }
}
...
bool ggml_validate_row_data(enum ggml_type type, const void * data, size_t nbytes) {
    ...
    switch (type) {
        ...
        case GGML_TYPE_Q2_SYM32K4: {
            VALIDATE_ROW_DATA_D_F16_IMPL(block_q2_sym32k4, data, nb);
        } break;
        case GGML_TYPE_QT_MS32K4: {
            VALIDATE_ROW_DATA_D_F16_IMPL(block_qt_ms32k4, data, nb);
            for (size_t i = 0; i < nb; ++i) {
                unsigned count = 0;
                for (size_t j = 0; j < sizeof(q[i].mask); ++j) {
                    unsigned bits = q[i].mask[j];
                    while (bits) {
                        bits &= bits - 1;
                        ++count;
                    }
                }
                if (count != QKT_MS32K_NZTOT) {
                    fprintf(stderr, "%s: invalid ternary mask at block %zu: %u nonzeros\n", __func__, i, count);
                    return false;
                }
            }
        } break;
        case GGML_TYPE_Q4_SYM16F: {
            VALIDATE_ROW_DATA_D_F16_IMPL(block_q4_sym16f, data, nb);
        } break;
