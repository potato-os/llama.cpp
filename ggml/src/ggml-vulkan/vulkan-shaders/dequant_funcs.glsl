...
// ---- Millie symmetric K-scale types ----
// dequantize()/dequantize4() return the value WITHOUT the fp16 super-scale d
// (sub-scale applied); get_dm() returns (d, 0) and the callers multiply.

#if defined(DATA_A_Q2_SYM32K4)
float sym_subscale(uint ib, uint s, uint a_offset) {
    const uint sc = uint(data_a[a_offset + ib].scales[s >> 1]);
    return float((s & 1) != 0 ? (sc >> 4) : (sc & 0xF));
}
vec2 dequantize(uint ib, uint iqs, uint a_offset) {
    // iqs is even, so both elements live in byte iqs/4
    const uint b = uint(data_a[a_offset + ib].qs[iqs / 4]);
    const uint sh = 2 * (iqs % 4);
    const float sc = sym_subscale(ib, iqs / 32, a_offset);
    return sc * (vec2((b >> sh) & 3, (b >> (sh + 2)) & 3) - 1.5f);
}
vec4 dequantize4(uint ib, uint iqs, uint a_offset) {
    // iqs is a multiple of 4: one byte holds the four elements
    const uint b = uint(data_a[a_offset + ib].qs[iqs / 4]);
    const float sc = sym_subscale(ib, iqs / 32, a_offset);
    return sc * (vec4(b & 3, (b >> 2) & 3, (b >> 4) & 3, (b >> 6) & 3) - 1.5f);
}
#endif
...
vec2 dequantize(uint ib, uint iqs, uint a_offset) {
    const uint b = uint(data_a[a_offset + ib].qs[iqs / 8]);
    const uint sh = iqs % 8;
    const float sc = sym_subscale(ib, iqs / 32, a_offset);
    return sc * (vec2((b >> sh) & 1, (b >> (sh + 1)) & 1) - 0.5f);
}
vec4 dequantize4(uint ib, uint iqs, uint a_offset) {
    const uint b = uint(data_a[a_offset + ib].qs[iqs / 8]);
    const uint sh = iqs % 8; // 0 or 4
    const float sc = sym_subscale(ib, iqs / 32, a_offset);
    return sc * (vec4((b >> sh) & 1, (b >> (sh + 1)) & 1, (b >> (sh + 2)) & 1, (b >> (sh + 3)) & 1) - 0.5f);
}
#endif

#if defined(DATA_A_Q4_SYM16K)
vec2 dequantize(uint ib, uint iqs, uint a_offset) {
    // pairwise nibbles: byte iqs/2 holds elems iqs (low) and iqs+1 (high)
    const uint b = uint(data_a[a_offset + ib].qs[iqs / 2]);
    const float sc = float(data_a[a_offset + ib].scales[iqs / 16]);
    return sc * (vec2(b & 0xF, b >> 4) - 7.5f);
}
vec4 dequantize4(uint ib, uint iqs, uint a_offset) {
    const uint u = uint(data_a_packed16[a_offset + ib].qs[iqs / 4]);
    const float sc = float(data_a[a_offset + ib].scales[iqs / 16]);
    return sc * (vec4(u & 0xF, (u >> 4) & 0xF, (u >> 8) & 0xF, (u >> 12) & 0xF) - 7.5f);
}
#endif

#if defined(DATA_A_QT_MS32K4)
float tern_subscale(uint ib, uint s, uint a_offset) {
    const uint sc = uint(data_a[a_offset + ib].scales[s >> 1]);
    return float((s & 1) != 0 ? (sc >> 4) : (sc & 0xF));
}
// popcount of all mask bits of block ib strictly before bit `bit`
uint tern_sign_base(uint ib, uint bit, uint a_offset) {
    uint idx = 0;
    const uint wlim = bit >> 4;
    [[unroll]] for (uint w = 0; w < 16; ++w) {
        if (w < wlim) {
            idx += bitCount(uint(data_a_packed16[a_offset + ib].mask[w]));
        }
    }
    return idx + bitCount(uint(data_a_packed16[a_offset + ib].mask[wlim]) & ((1u << (bit & 15)) - 1u));
}
float tern_sign(uint ib, uint idx, uint a_offset) { // +-1 for the idx-th non-zero
    return ((uint(data_a[a_offset + ib].signs[idx >> 3]) >> (idx & 7)) & 1u) != 0 ? -1.0f : 1.0f;
}
vec2 dequantize(uint ib, uint iqs, uint a_offset) {
    // iqs is even; both bits sit in mask byte iqs/8
    const float sc = tern_subscale(ib, iqs / 32, a_offset);
    const uint mb = uint(data_a[a_offset + ib].mask[iqs >> 3]);
    uint idx = tern_sign_base(ib, iqs, a_offset);
    vec2 v = vec2(0.0f);
    [[unroll]] for (uint e = 0; e < 2; ++e) {
        if (((mb >> ((iqs + e) & 7)) & 1u) != 0) {
            v[e] = sc * tern_sign(ib, idx, a_offset);
            idx++;
        }
    }
    return v;
...
vec4 dequantize4(uint ib, uint iqs, uint a_offset) {
    // iqs is a multiple of 4; all four bits sit in mask byte iqs/8
    const float sc = tern_subscale(ib, iqs / 32, a_offset);
    const uint mb = uint(data_a[a_offset + ib].mask[iqs >> 3]);
    uint idx = tern_sign_base(ib, iqs, a_offset);
    vec4 v = vec4(0.0f);
