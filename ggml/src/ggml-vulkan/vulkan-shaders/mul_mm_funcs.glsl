#if defined(COOPMAT) && defined(DATA_A_QT_MS32K4)
// Distributed register cache: each eight-lane group holds one superblock.
uint tern_masks[BM * 8 / BLOCK_SIZE];
uint tern_windows[BM * 8 / BLOCK_SIZE];
float tern_scales[BM * 8 / BLOCK_SIZE];

void prepare_tern_a(const uint pos_a, const uint row, const uint col) {
    const uint slot = col / (gl_WorkGroupSize.x / 8);
    const uint ib = (pos_a + col * p.stride_a / LOAD_VEC_A + row) / 64;
    const uint base = gl_SubgroupInvocationID & ~7u;
    const uint mask_word = uint(data_a_packed16[ib].mask[2 * row])
                         | (uint(data_a_packed16[ib].mask[2 * row + 1]) << 16);
    const uint count = bitCount(mask_word);
    uint prefix = count;
    [[unroll]] for (uint delta = 1; delta < 8; delta *= 2) {
        const uint prev = subgroupShuffle(prefix, base + ((row - delta) & 7u));
        if (row >= delta) prefix += prev;
    }
    const uint start = prefix - count;
    const uint wi = start >> 4;
    const uint sh = start & 15;
    const uint lo = uint(data_a_packed16[ib].signs[min(wi, 7u)])
                  | (uint(data_a_packed16[ib].signs[min(wi + 1, 7u)]) << 16);
    const uint hi = uint(data_a_packed16[ib].signs[min(wi + 2, 7u)]);
    const uint scb = uint(data_a[ib].scales[row >> 1]);
    tern_masks[slot] = mask_word;
    tern_windows[slot] = sh == 0 ? lo : (lo >> sh) | (hi << (32 - sh));
    tern_scales[slot] = float(data_a[ib].d) * float((row & 1) != 0 ? scb >> 4 : scb & 15);
}
#endif
...
void load_a_to_shmem(const uint pos_a, const uint row, const uint col, const uint idx_m, const uint block, const uint end_k) {
            ...
            const uint ib = idx / 64;            // 4 values per idx
            const uint iqs = (idx % 64) * 4;     // first element, multiple of 4
            const uint s = iqs / 32;

            const uint scb = uint(data_a[ib].scales[s >> 1]);
            const float sc = float((s & 1) != 0 ? (scb >> 4) : (scb & 0xF));
            const uint b = uint(data_a[ib].qs[iqs / 4]);
            const vec4 v = (float(data_a[ib].d) * sc) * (vec4(b & 3, (b >> 2) & 3, (b >> 4) & 3, (b >> 6) & 3) - 1.5f);
...
#ifdef COOPMAT
            if (BK == 32 && gl_SubgroupSize >= 8) {
                // Reuse the superblock cache for all eight K tiles.
                const uint lane = gl_SubgroupInvocationID;
                const uint base = lane & ~7u;
                const uint slot = col / (gl_WorkGroupSize.x / 8);
                const uint mask_bits = subgroupShuffle(tern_masks[slot], base + s);
                const uint window = subgroupShuffle(tern_windows[slot], base + s);
                const float dsc = subgroupShuffle(tern_scales[slot], base + s);
                vec4 v;
                [[unroll]] for (uint e = 0; e < 4; ++e) {
                    const uint bit = row * 4 + e;
                    const uint rank = bitCount(mask_bits & ((1u << bit) - 1u));
                    const uint nz = (mask_bits >> bit) & 1u;
                    const uint neg = (window >> rank) & 1u;
                    v[e] = dsc * float(int(nz) * (1 - 2 * int(neg)));
                }
                buf_a[buf_idx    ] = FLOAT_TYPEV2(v.xy);
                buf_a[buf_idx + 1] = FLOAT_TYPEV2(v.zw);
                return;
            ...
            // running sign index across the block
            uint sidx = 0;
            const uint wlim = iqs >> 4;
            [[unroll]] for (uint w = 0; w < 16; ++w) {
                if (w < wlim) {
                    sidx += bitCount(uint(data_a_packed16[ib].mask[w]));
                }
            }
            sidx += bitCount(uint(data_a_packed16[ib].mask[wlim]) & ((1u << (iqs & 15)) - 1u));

            const uint mb = uint(data_a[ib].mask[iqs >> 3]);
            vec4 v = vec4(0.0f);
            [[unroll]] for (uint e = 0; e < 4; ++e) {
                if (((mb >> ((iqs + e) & 7)) & 1u) != 0) {
                    v[e] = ((uint(data_a[ib].signs[sidx >> 3]) >> (sidx & 7)) & 1u) != 0 ? -dsc : dsc;
                    sidx++;
                }
            }
            ...
            const uint ib = idx / 64;            // 4 values per idx
            const uint iqs = (idx % 64) * 4;
            const uint s = iqs / 32;

            const uint scb = uint(data_a[ib].scales[s >> 1]);
            const float sc = float((s & 1) != 0 ? (scb >> 4) : (scb & 0xF));
            const uint b = uint(data_a[ib].qs[iqs / 8]);
            const uint sh = iqs % 8; // 0 or 4
            const vec4 v = (float(data_a[ib].d) * sc) * (vec4((b >> sh) & 1, (b >> (sh + 1)) & 1, (b >> (sh + 2)) & 1, (b >> (sh + 3)) & 1) - 0.5f);
            ...
            const uint iqs = (idx % 32) * 4;

            const float sc = float(data_a[ib].scales[iqs / 16]);
            const uint u = uint(data_a_packed16[ib].qs[iqs / 4]); // pairwise nibbles: sequential
            const vec4 v = (float(data_a[ib].d) * sc) * (vec4(u & 0xF, (u >> 4) & 0xF, (u >> 8) & 0xF, (u >> 12) & 0xF) - 7.5f);
