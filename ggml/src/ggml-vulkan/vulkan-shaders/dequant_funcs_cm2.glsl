...
// ---- Millie symmetric K-scale types. The product d * sub-scale * code is
// formed in fp32 and rounded to fp16 once. ----
#if defined(DATA_A_Q2_SYM32K4)
layout(buffer_reference, std430, buffer_reference_align = 2) buffer decodeBufQ2_SYM32K4 {
   block_q2_sym32k4 block;
};

float16_t dequantFuncQ2_SYM32K4(const in decodeBufQ2_SYM32K4 bl, const in uint blockCoords[2], const in uint coordInBlock[2])
{
    const uint idx = coordInBlock[1];
    const uint s = idx >> 5;
    const uint scb = uint(bl.block.scales[s >> 1]);
    const uint sc = (s & 1) != 0 ? (scb >> 4) : (scb & 0xF);
    const uint q = (uint(bl.block.qs[idx >> 2]) >> (2 * (idx & 3))) & 3;
    return float16_t(float(bl.block.d) * float(sc) * (float(q) - 1.5f));
}
#endif

#if defined(DATA_A_QT_MS32K4)
layout(buffer_reference, std430, buffer_reference_align = 2) buffer decodeBufQT_MS32K4 {
   block_qt_ms32k4 block;
};

layout(buffer_reference, std430, buffer_reference_align = 2) buffer decodeBufQT_MS32K4_packed16 {
   block_qt_ms32k4_packed16 block;
};

float16_t dequantFuncQT_MS32K4(const in decodeBufQT_MS32K4 bl, const in uint blockCoords[2], const in uint coordInBlock[2])
{
    decodeBufQT_MS32K4_packed16 bl16 = decodeBufQT_MS32K4_packed16(bl);
    const uint idx = coordInBlock[1];

    if (((uint(bl.block.mask[idx >> 3]) >> (idx & 7)) & 1u) == 0) {
    ...
    // sign index = popcount of preceding mask bits, across the whole block
    uint sidx = 0;
    const uint wlim = idx >> 4;
    [[unroll]] for (uint w = 0; w < 16; ++w) {
        if (w < wlim) {
            sidx += bitCount(uint(bl16.block.mask[w]));
        }
    }
    sidx += bitCount(uint(bl16.block.mask[wlim]) & ((1u << (idx & 15)) - 1u));

    const float sgn = ((uint(bl.block.signs[sidx >> 3]) >> (sidx & 7)) & 1u) != 0 ? -1.0f : 1.0f;
    return float16_t(float(bl.block.d) * float(sc) * sgn);
}
#endif

#if defined(DATA_A_Q1_SYM32K)
layout(buffer_reference, std430, buffer_reference_align = 2) buffer decodeBufQ1_SYM32K {
   block_q1_sym32k block;
};

float16_t dequantFuncQ1_SYM32K(const in decodeBufQ1_SYM32K bl, const in uint blockCoords[2], const in uint coordInBlock[2])
{
    const uint idx = coordInBlock[1];
    const uint s = idx >> 5;
    const uint scb = uint(bl.block.scales[s >> 1]);
    const uint sc = (s & 1) != 0 ? (scb >> 4) : (scb & 0xF);
    const uint bit = (uint(bl.block.qs[idx >> 3]) >> (idx & 7)) & 1;
    return float16_t(float(bl.block.d) * float(sc) * (float(bit) - 0.5f));
}
#endif

#if defined(DATA_A_Q4_SYM16K)
layout(buffer_reference, std430, buffer_reference_align = 2) buffer decodeBufQ4_SYM16K {
   block_q4_sym16k block;
};

float16_t dequantFuncQ4_SYM16K(const in decodeBufQ4_SYM16K bl, const in uint blockCoords[2], const in uint coordInBlock[2])
{
    const uint idx = coordInBlock[1];
    const float sc = float(bl.block.scales[idx >> 4]);
    const uint b = uint(bl.block.qs[idx >> 1]);
    const uint q = (idx & 1) != 0 ? (b >> 4) : (b & 0xF);
    return float16_t(float(bl.block.d) * sc * (float(q) - 7.5f));
}
#endif
...
#define dequantFuncA dequantFuncQ2_SYM32K4
#elif defined(DATA_A_QT_MS32K4)
#define dequantFuncA dequantFuncQT_MS32K4
#elif defined(DATA_A_Q1_SYM32K)
#define dequantFuncA dequantFuncQ1_SYM32K
#elif defined(DATA_A_Q4_SYM16K)
#define dequantFuncA dequantFuncQ4_SYM16K
