...
// Millie symmetric K-scale types: 32-element sub-blocks (16 for q4_sym16k) that
// line up with q8_1 blocks, so they use the k-quant integer-dot paths.
bool is_sym_quant(const std::string& type_name) {
    return type_name == "q2_sym32k4" || type_name == "q1_sym32k" || type_name == "q4_sym16k" || type_name == "qt_ms32k4";
}
...
compile_count_guard acquire_compile_slot() {
    ...
    if (const char * jobs = std::getenv("GGML_VULKAN_SHADER_JOBS")) {
        char * end = nullptr;
        const long value = std::strtol(jobs, &end, 10);
        if (end == jobs || *end != '\0' || value < 1 || value > 16) {
            throw std::runtime_error("GGML_VULKAN_SHADER_JOBS must be an integer from 1 to 16");
        }
...
void matmul_shaders(bool fp16, MatMulIdType matmul_id_type, bool coopmat, bool coopmat2, bool f16acc) {
    ...
    for (const auto& tname : type_names) {
        ...
        if ((tname == "q1_0") || (tname == "q4_0") || (tname == "q4_1") || (tname == "q5_1") || (tname == "iq1_s") || (tname == "iq1_m") || (tname == "iq2_xxs") || (tname == "iq2_xs") || (tname == "iq2_s"))
            load_vec_quant = "8";
        else if ((tname == "q5_0") || (tname == "q8_0") || (tname == "q2_k") || (tname == "q4_k") || (tname == "q5_k") || (tname == "iq3_xxs") || (tname == "iq3_s") || (tname == "iq4_xs") || (tname == "iq4_nl") || (tname == "mxfp4") || (tname == "nvfp4") || is_sym_quant(tname))
            load_vec_quant = "4";
        ...
        if (!f16acc && !coopmat && !coopmat2 && (is_legacy_quant(tname) || is_k_quant(tname) || tname == "mxfp4" || is_sym_quant(tname))) {
...
#if defined(GGML_VULKAN_INTEGER_DOT_GLSLC_SUPPORT)
        if (is_legacy_quant(tname) || tname == "mxfp4" || is_k_quant(tname) || tname == "iq1_s" || tname == "iq1_m" || is_sym_quant(tname)) {
...
void write_output_files() {
    ...
    for (const auto& tname : type_names) {
        if (btype == "q8_1" && !is_legacy_quant(tname) && tname != "mxfp4" && !is_k_quant(tname) && tname != "iq1_s" && tname != "iq1_m" && !is_sym_quant(tname)) {
            continue;
        }
