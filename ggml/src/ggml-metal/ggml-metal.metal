...
template <typename type4x4>
void dequantize_q4_sym16f(device const block_q4_sym16f * xb, short il, thread type4x4 & reg) {
    const float ds = (float)xb->d;
    device const uint8_t * qs = xb->qs;

    // Unpack adjacent low/high nibbles into each four-element row.
    float4x4 reg_f;
    FOR_UNROLL (short r = 0; r < 4; ++r) {
...
template [[host_name("kernel_get_rows_q4_sym16f")]] kernel get_rows_q_t kernel_get_rows_q<block_q4_sym16f, 1, dequantize_q4_sym16f>;
template [[host_name("kernel_mul_mm_q4_sym16f_f32")]] kernel mul_mm_t kernel_mul_mm<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_sym16f, 1,   dequantize_q4_sym16f, float, float4x4,  float, float2x4>;
template [[host_name("kernel_mul_mm_q4_sym16f_f16")]] kernel mul_mm_t kernel_mul_mm<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_sym16f, 1,   dequantize_q4_sym16f, float, float4x4,  half, half2x4>;
template [[host_name("kernel_mul_mm_id_q4_sym16f_f32")]] kernel mul_mm_id kernel_mul_mm_id<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_sym16f, 1,   dequantize_q4_sym16f, float, float4x4,  float, float2x4>;
template [[host_name("kernel_mul_mm_id_q4_sym16f_f16")]] kernel mul_mm_id kernel_mul_mm_id<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_sym16f, 1,   dequantize_q4_sym16f, float, float4x4,  half, half2x4>;

[[host_name("kernel_mul_mv_q4_sym16f_f32")]]
kernel void kernel_mul_mv_q4_sym16f_f32(
 constant ggml_metal_kargs_mul_mv & args,
 ...
 ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    const int row = tg.x*FC_mul_mv_nsg + sg;
    if (row >= args.ne0) return;
    const uint i12 = tg.z%args.ne12, i13=tg.z/args.ne12;
    device const block_q4_sym16f * x = (device const block_q4_sym16f *)(src0 + row*args.nb01 + (i12/args.r2)*args.nb02 + (i13/args.r3)*args.nb03);
    device const float * y = (device const float *)(src1 + tg.y*args.nb11 + i12*args.nb12 + i13*args.nb13);
    float sum=0;
    for (int b=lane;b<args.ne00/16;b+=32) {
        float acc=0;
        for (short j=0;j<8;++j) {
            const uchar q=x[b].qs[j];
            acc += ((q&15)-7.5f)*y[b*16+2*j]+((q>>4)-7.5f)*y[b*16+2*j+1];
        }
        sum += (float)x[b].d*acc;
    }
    sum=simd_sum(sum);
    if(lane==0) ((device float *)dst)[tg.z*args.ne0*args.ne1+tg.y*args.ne0+row]=sum;
}

[[host_name("kernel_mul_mv_q4_sym16f_f16")]]
kernel void kernel_mul_mv_q4_sym16f_f16(
 constant ggml_metal_kargs_mul_mv & args,
 device const char * src0, device const char * src1, device char * dst,
 threadgroup char * shmem [[threadgroup(0)]],
 uint3 tg [[threadgroup_position_in_grid]],
 ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    ...
    const uint i12 = tg.z%args.ne12, i13=tg.z/args.ne12;
    device const block_q4_sym16f * x = (device const block_q4_sym16f *)(src0 + row*args.nb01 + (i12/args.r2)*args.nb02 + (i13/args.r3)*args.nb03);
    device const half * y = (device const half *)(src1 + tg.y*args.nb11 + i12*args.nb12 + i13*args.nb13);
    float sum=0;
    for (int b=lane;b<args.ne00/16;b+=32) {
        float acc=0;
        for (short j=0;j<8;++j) {
