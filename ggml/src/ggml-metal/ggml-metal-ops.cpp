    ...
    if (op->src[1]->type == GGML_TYPE_F32 && (ne00%128 == 0) &&
        (
         (
          (
           ...
           op->src[0]->type == GGML_TYPE_Q4_SYM16 ||
           op->src[0]->type == GGML_TYPE_Q4_SYM16K ||
        ...
        switch (ne11) {
            ...
            case 7:
                r1ptg = 4; break;
            case 8:
                // E2a: single pass for 8 streams when the type has an r1_8 instance
                r1ptg = op->src[0]->type == GGML_TYPE_Q4_SYM16K ? 8 : 4; break;
            case 5:
                r1ptg = 5; break;
            default:
...
int ggml_metal_op_mul_mat_id(ggml_metal_op_t ctx, int idx) {
    ...
    // The custom ternary kernel reuses decoded blocks across a row tile,
    // allowing matrix-matrix dispatch at a smaller batch size.
    int ne21_mm_id_min = 32;
    if (op->src[0]->type == GGML_TYPE_QT_MS32K4) {
        const char * e = getenv("GGML_TERN_MMID_MIN");
        ne21_mm_id_min = e ? atoi(e) : 8;
    }
...
// multi-query vec FA eligibility: >=4 query rows over a long f16 KV with the
// dk=dv=256 decode shape. Shared by the vec/block router below and the vec
// dispatch section so they cannot disagree.
static bool ggml_metal_op_flash_attn_ext_use_mq(const ggml_tensor * op) {
    ...
    // Optional vector-unit multi-query path. The default block kernel
    // shares KV reads across an eight-query tile using simdgroup matrices.
    if (getenv("GGML_METAL_FA_MQ") == nullptr) {
        return false;
    }

    return ne01 >= 4 && ne01 < 20 && ne11 >= 16384 &&
        op->src[3] != nullptr &&  // mask
        op->src[4] == nullptr &&  // no sinks
...
bool ggml_metal_op_flash_attn_ext_use_vec(const ggml_tensor * op) {
    ...
    const int64_t ne11 = op->src[1]->ne[1]; // KV sequence length

    // Use the vector kernel for small batches with supported head sizes.
    // For >=4 query rows over >=16k KV cells, the block kernel shares KV
    // reads across a query block. The optional multi-query path uses vec dispatch.
    if (ggml_metal_op_flash_attn_ext_use_mq(op)) {
        return true;
    }

    return (ne01 < 20) && !(ne01 >= 4 && ne11 >= 16384) && (ne00 % 32 == 0);
}
...
#undef FATTN_SMEM
        ...
        // half4x4 kernel
        // multi-query variant: stream each K value once for 4 query rows. Engaged
        // for multi-stream decode over long KV with the plain f16 dk=dv=256 shape
        // supported here; other shapes keep the single-query path.
        const bool use_mq = ggml_metal_op_flash_attn_ext_use_mq(op);

        const int nqptg = use_mq ? 4 : OP_FLASH_ATTN_EXT_VEC_NQPSG; // queries per threadgroup
...
#define FATTN_SMEM(nsg) (GGML_PAD(((GGML_PAD(ne00, 128) + 4*ncpsg + 2*GGML_PAD(ne20, 128))*(nsg))*(sizeof(float)/2), 16))
        ...
        if (false) {
            ...
            // multi-query: shared memory scales with nqptg; keep nsg low so
            // threadgroup memory stays small enough for good occupancy
            if (use_mq) {
                nsg = 1;
            }
        }
        ...
        const size_t smem = nqptg*FATTN_SMEM(nsg); // per-simdgroup regions repeat per query row
