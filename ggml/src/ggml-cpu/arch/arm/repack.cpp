...
// Q4_SYM16 x4 tile: chunk c of the interleaved qs covers, for each of 4 rows,
// source bytes 4c..4c+3 -> group g=c/2, positions 4(c%2)..+3; low nibbles pair
// with activation 4-lane (c%2) of group vector, high nibbles with lane 2+(c%2).
// Levels via signed odd LUT (2*code-15); the 0.5 rides in the final scale.
void ggml_gemv_q4_sym16_4x4_q8_0(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc) {
    const int qk = QK8_0;
    const int nb = n / qk;
    ...
    const block_q4_sym16x4 * b_ptr = (const block_q4_sym16x4 *) vx;

    for (int c = 0; c < nc; c += 4) {
        const block_q8_0 * a_ptr = (const block_q8_0 *) vy;
        float32x4_t acc = vdupq_n_f32(0);
        for (int b = 0; b < nb; b++) {
            const int8x16_t a0 = vld1q_s8(a_ptr->qs);      // group0 elems 0..15
            const int8x16_t a1 = vld1q_s8(a_ptr->qs + 16); // group1 elems 16..31
            const float d8h = 0.5f * GGML_CPU_FP16_TO_FP32(a_ptr->d);

            const uint8x16_t q0 = vld1q_u8(b_ptr->qs);
            const uint8x16_t q1 = vld1q_u8(b_ptr->qs + 16);
            const uint8x16_t q2 = vld1q_u8(b_ptr->qs + 32);
            const uint8x16_t q3 = vld1q_u8(b_ptr->qs + 48);

            int32x4_t ig0 = vdupq_n_s32(0);
            int32x4_t ig1 = vdupq_n_s32(0);

            ig0 = vdotq_laneq_s32(ig0, vqtbl1q_s8(lut, vandq_u8  (q0, m4b)), a0, 0);
            ig0 = vdotq_laneq_s32(ig0, vqtbl1q_s8(lut, vshrq_n_u8(q0, 4)),   a0, 2);
            ig0 = vdotq_laneq_s32(ig0, vqtbl1q_s8(lut, vandq_u8  (q1, m4b)), a0, 1);
            ig0 = vdotq_laneq_s32(ig0, vqtbl1q_s8(lut, vshrq_n_u8(q1, 4)),   a0, 3);

            ig1 = vdotq_laneq_s32(ig1, vqtbl1q_s8(lut, vandq_u8  (q2, m4b)), a1, 0);
            ig1 = vdotq_laneq_s32(ig1, vqtbl1q_s8(lut, vshrq_n_u8(q2, 4)),   a1, 2);
            ig1 = vdotq_laneq_s32(ig1, vqtbl1q_s8(lut, vandq_u8  (q3, m4b)), a1, 1);
            ig1 = vdotq_laneq_s32(ig1, vqtbl1q_s8(lut, vshrq_n_u8(q3, 4)),   a1, 3);

            const float32x4_t s0 = {
                ggml_table_ue4m3_f32[b_ptr->d[0]], ggml_table_ue4m3_f32[b_ptr->d[1]],
                ggml_table_ue4m3_f32[b_ptr->d[2]], ggml_table_ue4m3_f32[b_ptr->d[3]] };
            const float32x4_t s1 = {
                ggml_table_ue4m3_f32[b_ptr->d[4]], ggml_table_ue4m3_f32[b_ptr->d[5]],
                ggml_table_ue4m3_f32[b_ptr->d[6]], ggml_table_ue4m3_f32[b_ptr->d[7]] };

            acc = vfmaq_f32(acc, vcvtq_f32_s32(ig0), vmulq_n_f32(s0, d8h));
            acc = vfmaq_f32(acc, vcvtq_f32_s32(ig1), vmulq_n_f32(s1, d8h));
    ...
    return;
#endif
    ggml_gemv_q4_sym16_4x4_q8_0_generic(n, s, bs, vx, vy, nr, nc);
}

void ggml_gemm_q4_sym16_4x4_q8_0(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc) {
    const int qk = QK8_0;
    const int nb = n / qk;
    ...
    for (int y = 0; y < nr / 4; y++) {
        const block_q8_0x4 * a_ptr_base = (const block_q8_0x4 *) vy + (y * nb);
        for (int x = 0; x < nc / 4; x++) {
            const block_q4_sym16x4 * b_ptr = (const block_q4_sym16x4 *) vx + (x * nb);
            float32x4_t facc[4] = { vdupq_n_f32(0), vdupq_n_f32(0), vdupq_n_f32(0), vdupq_n_f32(0) };
            for (int l = 0; l < nb; l++) {
                const uint8x16_t q0 = vld1q_u8(b_ptr[l].qs);
                const uint8x16_t q1 = vld1q_u8(b_ptr[l].qs + 16);
                const uint8x16_t q2 = vld1q_u8(b_ptr[l].qs + 32);
                const uint8x16_t q3 = vld1q_u8(b_ptr[l].qs + 48);

                // w[c][0] = low nibbles of chunk c, w[c][1] = high nibbles
                const int8x16_t w00 = vqtbl1q_s8(lut, vandq_u8  (q0, m4b));
                const int8x16_t w01 = vqtbl1q_s8(lut, vshrq_n_u8(q0, 4));
                const int8x16_t w10 = vqtbl1q_s8(lut, vandq_u8  (q1, m4b));
                const int8x16_t w11 = vqtbl1q_s8(lut, vshrq_n_u8(q1, 4));
                const int8x16_t w20 = vqtbl1q_s8(lut, vandq_u8  (q2, m4b));
                const int8x16_t w21 = vqtbl1q_s8(lut, vshrq_n_u8(q2, 4));
                const int8x16_t w30 = vqtbl1q_s8(lut, vandq_u8  (q3, m4b));
                const int8x16_t w31 = vqtbl1q_s8(lut, vshrq_n_u8(q3, 4));

                // av[je] = the 4 activation columns' elems 4*je..4*je+3
                int8x16_t av[8];
                for (int je = 0; je < 8; je++) {
                    av[je] = vld1q_s8(a_ptr_base[l].qs + 16*je);
                }

                const float32x4_t s0 = {
                    ggml_table_ue4m3_f32[b_ptr[l].d[0]], ggml_table_ue4m3_f32[b_ptr[l].d[1]],
                    ggml_table_ue4m3_f32[b_ptr[l].d[2]], ggml_table_ue4m3_f32[b_ptr[l].d[3]] };
                const float32x4_t s1 = {
                    ggml_table_ue4m3_f32[b_ptr[l].d[4]], ggml_table_ue4m3_f32[b_ptr[l].d[5]],
                    ggml_table_ue4m3_f32[b_ptr[l].d[6]], ggml_table_ue4m3_f32[b_ptr[l].d[7]] };

                // vdotq_laneq_s32 needs a constant lane -> unroll the column loop
#define GGML_SYM16_GEMM_COL(m)                                                            \
                do {                                                                      \
                    int32x4_t ig0 = vdupq_n_s32(0);                                       \
                    int32x4_t ig1 = vdupq_n_s32(0);                                       \
                    /* group0: chunks 0,1 -> lo lanes 0/1, hi lanes 2/3 (av[0..3]) */     \
                    ig0 = vdotq_laneq_s32(ig0, w00, av[0], m);                            \
                    ig0 = vdotq_laneq_s32(ig0, w01, av[2], m);                            \
                    ig0 = vdotq_laneq_s32(ig0, w10, av[1], m);                            \
                    ig0 = vdotq_laneq_s32(ig0, w11, av[3], m);                            \
                    /* group1: chunks 2,3 over elems 16..31 (av[4..7]) */                 \
                    ig1 = vdotq_laneq_s32(ig1, w20, av[4], m);                            \
                    ig1 = vdotq_laneq_s32(ig1, w21, av[6], m);                            \
                    ig1 = vdotq_laneq_s32(ig1, w30, av[5], m);                            \
                    ig1 = vdotq_laneq_s32(ig1, w31, av[7], m);                            \
                    const float d8h = 0.5f * GGML_CPU_FP16_TO_FP32(a_ptr_base[l].d[m]);   \
                    facc[m] = vfmaq_f32(facc[m], vcvtq_f32_s32(ig0), vmulq_n_f32(s0, d8h)); \
                    facc[m] = vfmaq_f32(facc[m], vcvtq_f32_s32(ig1), vmulq_n_f32(s1, d8h)); \
                } while (0)

                GGML_SYM16_GEMM_COL(0);
                GGML_SYM16_GEMM_COL(1);
                GGML_SYM16_GEMM_COL(2);
                GGML_SYM16_GEMM_COL(3);
#undef GGML_SYM16_GEMM_COL
            }
            for (int m = 0; m < 4; m++) {
                vst1q_f32(s + (y * 4 + m) * bs + x * 4, facc[m]);
            }
        }
    }
    return;
#endif
    ggml_gemm_q4_sym16_4x4_q8_0_generic(n, s, bs, vx, vy, nr, nc);
}

// Q4_SYM16 x4 interleave-8: an 8-byte chunk of a row is one full 16-elem scale
// group (bytes 8g..8g+7: low nibbles -> elems 16g..+7, high -> 16g+8..+15).
// qs layout: [r0 g0(8B), r1 g0, r2 g0, r3 g0, r0 g1, ...].
void ggml_gemv_q4_sym16_4x8_q8_0(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc) {
    const int qk = QK8_0;
    const int nb = n / qk;
    ...
    for (int c = 0; c < nc; c += 4) {
        ...
        float32x4_t acc = vdupq_n_f32(0);
        for (int b = 0; b < nb; b++) {
            // y "rep" vectors: group g's low-half elems duplicated across both
            // 8-byte halves (pairs with [row_r seg, row_{r+1} seg] weights)
            const int8x8_t ylo0 = vld1_s8(a_ptr->qs);      // elems 0..7
            const int8x8_t yhi0 = vld1_s8(a_ptr->qs + 8);  // elems 8..15
            const int8x8_t ylo1 = vld1_s8(a_ptr->qs + 16); // elems 16..23
            const int8x8_t yhi1 = vld1_s8(a_ptr->qs + 24); // elems 24..31
            const int8x16_t yrl0 = vcombine_s8(ylo0, ylo0);
            const int8x16_t yrh0 = vcombine_s8(yhi0, yhi0);
            const int8x16_t yrl1 = vcombine_s8(ylo1, ylo1);
            const int8x16_t yrh1 = vcombine_s8(yhi1, yhi1);
            const float d8h = 0.5f * GGML_CPU_FP16_TO_FP32(a_ptr->d);

            const uint8x16_t q01_0 = vld1q_u8(b_ptr->qs);      // rows 0,1 group0
            const uint8x16_t q23_0 = vld1q_u8(b_ptr->qs + 16); // rows 2,3 group0
            const uint8x16_t q01_1 = vld1q_u8(b_ptr->qs + 32); // rows 0,1 group1
            const uint8x16_t q23_1 = vld1q_u8(b_ptr->qs + 48); // rows 2,3 group1

            // acc lanes after pairwise: [r0, r1, r2, r3] per group
            int32x4_t p01_0 = ggml_vdotq_s32(vdupq_n_s32(0), vqtbl1q_s8(lut, vandq_u8  (q01_0, m4b)), yrl0);
            p01_0 = ggml_vdotq_s32(p01_0, vqtbl1q_s8(lut, vshrq_n_u8(q01_0, 4)), yrh0);
            int32x4_t p23_0 = ggml_vdotq_s32(vdupq_n_s32(0), vqtbl1q_s8(lut, vandq_u8  (q23_0, m4b)), yrl0);
            p23_0 = ggml_vdotq_s32(p23_0, vqtbl1q_s8(lut, vshrq_n_u8(q23_0, 4)), yrh0);
            int32x4_t p01_1 = ggml_vdotq_s32(vdupq_n_s32(0), vqtbl1q_s8(lut, vandq_u8  (q01_1, m4b)), yrl1);
            p01_1 = ggml_vdotq_s32(p01_1, vqtbl1q_s8(lut, vshrq_n_u8(q01_1, 4)), yrh1);
            int32x4_t p23_1 = ggml_vdotq_s32(vdupq_n_s32(0), vqtbl1q_s8(lut, vandq_u8  (q23_1, m4b)), yrl1);
            p23_1 = ggml_vdotq_s32(p23_1, vqtbl1q_s8(lut, vshrq_n_u8(q23_1, 4)), yrh1);

            const int32x4_t ig0 = vpaddq_s32(p01_0, p23_0); // [r0,r1,r2,r3] group0
            const int32x4_t ig1 = vpaddq_s32(p01_1, p23_1); // [r0,r1,r2,r3] group1
    ...
    return;
#endif
    ggml_gemv_q4_sym16_4x8_q8_0_generic(n, s, bs, vx, vy, nr, nc);
}

void ggml_gemm_q4_sym16_4x8_q8_0(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc) {
    const int qk = QK8_0;
    const int nb = n / qk;
    ...
    for (int y = 0; y < nr / 4; y++) {
        ...
        for (int x = 0; x < nc / 4; x++) {
            const block_q4_sym16x4 * b_ptr = (const block_q4_sym16x4 *) vx + (x * nb);
            // facc[rp][cp]: 2x2 output tiles, lanes [w(2rp)c(2cp), w(2rp)c(2cp+1), w(2rp+1)c(2cp), w(2rp+1)c(2cp+1)]
            float32x4_t facc[2][2] = { { vdupq_n_f32(0), vdupq_n_f32(0) }, { vdupq_n_f32(0), vdupq_n_f32(0) } };
            for (int l = 0; l < nb; l++) {
                const uint8x16_t q01_0 = vld1q_u8(b_ptr[l].qs);
                const uint8x16_t q23_0 = vld1q_u8(b_ptr[l].qs + 16);
                const uint8x16_t q01_1 = vld1q_u8(b_ptr[l].qs + 32);
                const uint8x16_t q23_1 = vld1q_u8(b_ptr[l].qs + 48);

                // W[rp][g][half]: [row 2rp seg(8B), row 2rp+1 seg] after LUT
                const int8x16_t w01_0l = vqtbl1q_s8(lut, vandq_u8  (q01_0, m4b));
                const int8x16_t w01_0h = vqtbl1q_s8(lut, vshrq_n_u8(q01_0, 4));
                const int8x16_t w23_0l = vqtbl1q_s8(lut, vandq_u8  (q23_0, m4b));
                const int8x16_t w23_0h = vqtbl1q_s8(lut, vshrq_n_u8(q23_0, 4));
                const int8x16_t w01_1l = vqtbl1q_s8(lut, vandq_u8  (q01_1, m4b));
                const int8x16_t w01_1h = vqtbl1q_s8(lut, vshrq_n_u8(q01_1, 4));
                const int8x16_t w23_1l = vqtbl1q_s8(lut, vandq_u8  (q23_1, m4b));
                const int8x16_t w23_1h = vqtbl1q_s8(lut, vshrq_n_u8(q23_1, 4));

                // activations: seg j (8 elems), col pair cp: [col 2cp seg, col 2cp+1 seg]
                // group g low half -> j = 2g, high half -> j = 2g+1
                const int8_t * aq = a_ptr_base[l].qs;
                const int8x16_t a01_0 = vld1q_s8(aq);       // cols 0,1 elems 0..7
                const int8x16_t a23_0 = vld1q_s8(aq + 16);  // cols 2,3 elems 0..7
                const int8x16_t a01_1 = vld1q_s8(aq + 32);  // cols 0,1 elems 8..15
                const int8x16_t a23_1 = vld1q_s8(aq + 48);
                const int8x16_t a01_2 = vld1q_s8(aq + 64);  // cols 0,1 elems 16..23
                const int8x16_t a23_2 = vld1q_s8(aq + 80);
                const int8x16_t a01_3 = vld1q_s8(aq + 96);  // cols 0,1 elems 24..31
                const int8x16_t a23_3 = vld1q_s8(aq + 112);

                // int accumulators per (rp, cp, g); mmla: acc = W(2x8) x A(2x8)^T
                int32x4_t ig[2][2][2];
                ig[0][0][0] = vmmlaq_s32(vmmlaq_s32(vdupq_n_s32(0), w01_0l, a01_0), w01_0h, a01_1);
                ig[0][1][0] = vmmlaq_s32(vmmlaq_s32(vdupq_n_s32(0), w01_0l, a23_0), w01_0h, a23_1);
                ig[1][0][0] = vmmlaq_s32(vmmlaq_s32(vdupq_n_s32(0), w23_0l, a01_0), w23_0h, a01_1);
                ig[1][1][0] = vmmlaq_s32(vmmlaq_s32(vdupq_n_s32(0), w23_0l, a23_0), w23_0h, a23_1);
                ig[0][0][1] = vmmlaq_s32(vmmlaq_s32(vdupq_n_s32(0), w01_1l, a01_2), w01_1h, a01_3);
                ig[0][1][1] = vmmlaq_s32(vmmlaq_s32(vdupq_n_s32(0), w01_1l, a23_2), w01_1h, a23_3);
                ig[1][0][1] = vmmlaq_s32(vmmlaq_s32(vdupq_n_s32(0), w23_1l, a01_2), w23_1h, a01_3);
                ig[1][1][1] = vmmlaq_s32(vmmlaq_s32(vdupq_n_s32(0), w23_1l, a23_2), w23_1h, a23_3);

                // scales: per (rp, cp, g): [sw(2rp,g)*da(2cp), sw(2rp,g)*da(2cp+1), sw(2rp+1,g)*da(2cp), ...]
                const float da0 = 0.5f * GGML_CPU_FP16_TO_FP32(a_ptr_base[l].d[0]);
                const float da1 = 0.5f * GGML_CPU_FP16_TO_FP32(a_ptr_base[l].d[1]);
                const float da2 = 0.5f * GGML_CPU_FP16_TO_FP32(a_ptr_base[l].d[2]);
                const float da3 = 0.5f * GGML_CPU_FP16_TO_FP32(a_ptr_base[l].d[3]);
                const float sw[2][4] = {
                    { ggml_table_ue4m3_f32[b_ptr[l].d[0]], ggml_table_ue4m3_f32[b_ptr[l].d[1]],
                      ggml_table_ue4m3_f32[b_ptr[l].d[2]], ggml_table_ue4m3_f32[b_ptr[l].d[3]] },
                    { ggml_table_ue4m3_f32[b_ptr[l].d[4]], ggml_table_ue4m3_f32[b_ptr[l].d[5]],
                ...
                const float dav[2][2] = { { da0, da1 }, { da2, da3 } };

                for (int rp = 0; rp < 2; rp++) {
                    for (int cp = 0; cp < 2; cp++) {
                        for (int g = 0; g < 2; g++) {
                            const float32x4_t sc = {
                                sw[g][2*rp    ] * dav[cp][0],
                                sw[g][2*rp    ] * dav[cp][1],
                                sw[g][2*rp + 1] * dav[cp][0],
                                sw[g][2*rp + 1] * dav[cp][1],
                            };
                            facc[rp][cp] = vfmaq_f32(facc[rp][cp], vcvtq_f32_s32(ig[rp][cp][g]), sc);
                        }
                    }
                }
            }
            for (int rp = 0; rp < 2; rp++) {
                for (int cp = 0; cp < 2; cp++) {
                    // lanes: [w(2rp)c(2cp), w(2rp)c(2cp+1), w(2rp+1)c(2cp), w(2rp+1)c(2cp+1)]
                    s[(y*4 + 2*cp    ) * bs + x*4 + 2*rp    ] = vgetq_lane_f32(facc[rp][cp], 0);
                    s[(y*4 + 2*cp + 1) * bs + x*4 + 2*rp    ] = vgetq_lane_f32(facc[rp][cp], 1);
                    s[(y*4 + 2*cp    ) * bs + x*4 + 2*rp + 1] = vgetq_lane_f32(facc[rp][cp], 2);
                    s[(y*4 + 2*cp + 1) * bs + x*4 + 2*rp + 1] = vgetq_lane_f32(facc[rp][cp], 3);
                }
            }
        }
    }
    return;
#endif
    ggml_gemm_q4_sym16_4x8_q8_0_generic(n, s, bs, vx, vy, nr, nc);
}
