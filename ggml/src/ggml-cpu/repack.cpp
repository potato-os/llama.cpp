...
// Q4_SYM16 x4 repack: per source byte j (0..15) of row r: group g=j/8, pos p=j%8,
// low nibble -> elem 16g+p, high nibble -> elem 16g+8+p, W = ue4m3(d[g])*(code-7.5).
// Interleaved layout: qs[16*(j/4) + 4*r + (j%4)], d[r]=row r group0, d[4+r]=group1.
void ggml_gemv_q4_sym16_4x4_q8_0_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc) {
    const int qk = QK8_0;
    const int nb = n / qk;
    ...
    for (int c = 0; c < nc; c += 4) {
        ...
        for (int b = 0; b < nb; b++) {
            ...
            for (int r = 0; r < 4; r++) {
                int sumi[2] = { 0, 0 };
                for (int j = 0; j < 16; j++) {
                    const uint8_t byte = b_ptr->qs[16*(j/4) + 4*r + (j%4)];
                    const int g = j/8, p = j%8;
                    sumi[g] += (2*(byte & 0x0F) - 15) * a_ptr->qs[16*g + p];
                    sumi[g] += (2*(byte >>   4) - 15) * a_ptr->qs[16*g + p + 8];
                }
                sumf[r] += d8h * (ggml_table_ue4m3_f32[b_ptr->d[r]]     * (float) sumi[0] +
                                  ggml_table_ue4m3_f32[b_ptr->d[4 + r]] * (float) sumi[1]);
            }
            a_ptr++;
            b_ptr++;
        }
...
void ggml_gemm_q4_sym16_4x4_q8_0_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc) {
    const int qk = QK8_0;
    const int nb = n / qk;
    ...
    for (int y = 0; y < nr / 4; y++) {
        ...
        for (int x = 0; x < nc / 4; x++) {
            ...
            for (int m = 0; m < 4; m++) {
                for (int j = 0; j < 4; j++) sumf[m][j] = 0.0f;
            }
            for (int l = 0; l < nb; l++) {
                for (int m = 0; m < 4; m++) {
                    const float d8h = 0.5f * GGML_CPU_FP16_TO_FP32(a_ptr[l].d[m]);
                    for (int j = 0; j < 4; j++) {
                        int sumi[2] = { 0, 0 };
                        for (int jj = 0; jj < 16; jj++) {
                            const uint8_t byte = b_ptr[l].qs[16*(jj/4) + 4*j + (jj%4)];
                            const int g = jj/8, p = jj%8;
                            const int elo = 16*g + p;
                            const int ehi = elo + 8;
                            // activation elem e of column m lives at qs[16*(e/4) + 4*m + e%4]
                            sumi[g] += (2*(byte & 0x0F) - 15) * a_ptr[l].qs[16*(elo/4) + 4*m + elo%4];
                            sumi[g] += (2*(byte >>   4) - 15) * a_ptr[l].qs[16*(ehi/4) + 4*m + ehi%4];
                        }
                        sumf[m][j] += d8h * (ggml_table_ue4m3_f32[b_ptr[l].d[j]]     * (float) sumi[0] +
                                             ggml_table_ue4m3_f32[b_ptr[l].d[4 + j]] * (float) sumi[1]);
                    }
                }
            }
            for (int m = 0; m < 4; m++) {
                for (int j = 0; j < 4; j++) {
                    s[(y * 4 + m) * bs + x * 4 + j] = sumf[m][j];
                }
            }
        }
    }
...
// Q4_SYM16 x4 interleave-8 variant: source byte j (0..15) of row r lives at
// qs[32*(j/8) + 8*r + (j%8)] -- an 8-byte chunk is one full scale group.
void ggml_gemv_q4_sym16_4x8_q8_0_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc) {
    const int qk = QK8_0;
    const int nb = n / qk;
    ...
    for (int c = 0; c < nc; c += 4) {
        ...
        for (int b = 0; b < nb; b++) {
            ...
            for (int r = 0; r < 4; r++) {
                int sumi[2] = { 0, 0 };
                for (int j = 0; j < 16; j++) {
                    const uint8_t byte = b_ptr->qs[32*(j/8) + 8*r + (j%8)];
                    const int g = j/8, p = j%8;
                    sumi[g] += (2*(byte & 0x0F) - 15) * a_ptr->qs[16*g + p];
                    sumi[g] += (2*(byte >>   4) - 15) * a_ptr->qs[16*g + p + 8];
                }
...
void ggml_gemm_q4_sym16_4x8_q8_0_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc) {
    const int qk = QK8_0;
    const int nb = n / qk;
            ...
            for (int l = 0; l < nb; l++) {
                for (int m = 0; m < 4; m++) {
                    ...
                    for (int j = 0; j < 4; j++) {
                        int sumi[2] = { 0, 0 };
                        for (int jj = 0; jj < 16; jj++) {
                            const uint8_t byte = b_ptr[l].qs[32*(jj/8) + 8*j + (jj%8)];
                            const int g = jj/8, p = jj%8;
                            const int elo = 16*g + p;
                            const int ehi = elo + 8;
                            // interleave-8 activations: elem e of column m at qs[32*(e/8) + 8*m + e%8]
                            sumi[g] += (2*(byte & 0x0F) - 15) * a_ptr[l].qs[32*(elo/8) + 8*m + elo%8];
                            sumi[g] += (2*(byte >>   4) - 15) * a_ptr[l].qs[32*(ehi/8) + 8*m + ehi%8];
                        }
                        sumf[m][j] += d8h * (ggml_table_ue4m3_f32[b_ptr[l].d[j]]     * (float) sumi[0] +
                                             ggml_table_ue4m3_f32[b_ptr[l].d[4 + j]] * (float) sumi[1]);
                    }
...
static int repack_q4_sym16_to_q4_sym16_4_bl(struct ggml_tensor * t, int interleave_block, const void * GGML_RESTRICT data, size_t data_size) {
    GGML_ASSERT(t->type == GGML_TYPE_Q4_SYM16);
    GGML_ASSERT(interleave_block == 4 || interleave_block == 8);
    constexpr int nrows_interleaved = 4;

    block_q4_sym16x4 * dst = (block_q4_sym16x4 *) t->data;
    const block_q4_sym16 * src = (const block_q4_sym16 *) data;
    int nrow = ggml_nrows(t);
    int nblocks = t->ne[0] / QK4_SYM16;

    GGML_ASSERT(data_size == (size_t) nrow * nblocks * sizeof(block_q4_sym16));

    if (t->ne[1] % nrows_interleaved != 0 || t->ne[0] % QK4_SYM16 != 0) {
        return -1;
    }

    for (int b = 0; b < nrow; b += nrows_interleaved) {
        for (int64_t x = 0; x < nblocks; x++) {
            block_q4_sym16x4 out;
            for (int r = 0; r < nrows_interleaved; r++) {
                const block_q4_sym16 * sb = &src[r * nblocks + x];
                out.d[r]     = sb->d[0];
                out.d[4 + r] = sb->d[1];
                if (interleave_block == 4) {
                    for (int c = 0; c < 4; c++) {
                        memcpy(out.qs + 16*c + 4*r, sb->qs + 4*c, 4);
                    }
                } else {
                    for (int c = 0; c < 2; c++) {
                        memcpy(out.qs + 32*c + 8*r, sb->qs + 8*c, 8);
                    }
                }
            }
            *dst++ = out;
...
template <> int repack<block_q4_sym16, 4, 4>(struct ggml_tensor * t, const void * data, size_t data_size) {
    return repack_q4_sym16_to_q4_sym16_4_bl(t, 4, data, data_size);
}

template <> int repack<block_q4_sym16, 8, 4>(struct ggml_tensor * t, const void * data, size_t data_size) {
    return repack_q4_sym16_to_q4_sym16_4_bl(t, 8, data, data_size);
}
...
template <> void gemv<block_q4_sym16, 4, 4, GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    ggml_gemv_q4_sym16_4x4_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemv<block_q4_sym16, 8, 4, GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    ggml_gemv_q4_sym16_4x8_q8_0(n, s, bs, vx, vy, nr, nc);
}
...
template <> void gemm<block_q4_sym16, 4, 4, GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    ggml_gemm_q4_sym16_4x4_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemm<block_q4_sym16, 8, 4, GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    ggml_gemm_q4_sym16_4x8_q8_0(n, s, bs, vx, vy, nr, nc);
}
...
static const ggml::cpu::tensor_traits * ggml_repack_get_optimal_repack_type(const struct ggml_tensor * cur) {
    ...
    // instance for Q4_SYM16
    static const ggml::cpu::repack::tensor_traits<block_q4_sym16, 4, 4, GGML_TYPE_Q8_0> q4_sym16_4x4_q8_0;
    static const ggml::cpu::repack::tensor_traits<block_q4_sym16, 8, 4, GGML_TYPE_Q8_0> q4_sym16_4x8_q8_0;
    ...
    } else if (cur->type == GGML_TYPE_Q4_SYM16) {
        if (ggml_cpu_has_neon() && ggml_cpu_has_matmul_int8()) {
            if (cur->ne[1] % 4 == 0) {
                return &q4_sym16_4x8_q8_0;
            }
        }
        if (ggml_cpu_has_neon() && ggml_cpu_has_dotprod()) {
            if (cur->ne[1] % 4 == 0) {
                return &q4_sym16_4x4_q8_0;
            }
        }
