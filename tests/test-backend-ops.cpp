...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // multi-query vec FA path (Metal: >=4 query rows over >=16k f16 KV, dk=dv=256)
    for (int nb : { 4, 5, 8, 12 }) {
        test_cases.emplace_back(new test_flash_attn_ext(256, 256, 2, {8, 1}, 16384, nb, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16));
    }
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_perf() {
 ...
 // High resolution output (large NPQ)
    ...
    // decode shapes: 32 heads d=128, 1 token/seq, multi-stream batch decode
    test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 32, 128, 1, 8));   // B=8 decode
    test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 32, 128, 1, 12));  // B=12 decode
    test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 32, 128, 512, 8)); // prefill ubatch, 8 seqs
