...
void common_chat_peg_mapper::map(const common_peg_ast_node & node) {
    ...
    if (is_tool_args && current_tool) {
        ...
        if (!text.empty() && text.front() == '{') {
            ...
            // Arguments as a JSON-encoded string literal: decode to the
            // underlying JSON text. Incomplete strings (streaming partials)
            // fail to parse and are skipped until the node is complete.
            try {
                args_target() = nlohmann::json::parse(text).get<std::string>();
            } catch (const std::exception &) {
                // keep previous value until the full string literal arrives
            }
        }
    }
