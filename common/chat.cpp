...
// Millie-native format: <think> reasoning, free text, then tool calls as one
// compact JSON object per <tool_call> block --
//   {"name":"NAME","arguments":"<JSON-encoded string>","call_id":"call_..."}
// One PEG definition yields both the parser and the lazy grammar; arguments
// are enforced as a JSON string (inner schema violations surface as tool
// errors rather than as parse failures).
static common_chat_params common_chat_params_init_millie_native(const common_chat_template & tmpl,
                                                                const autoparser::generation_params & inputs) {
    ...
    auto parser = build_chat_peg_parser([&](common_chat_peg_builder & p) {
        ...
        // The model occasionally opens a tool call without closing </think>;
        // treat <tool_call> as an alternate end of the reasoning region so
        // those calls still parse (the close tag becomes optional).
        auto reasoning = extract_reasoning
                             ? p.optional("<think>" + p.reasoning(p.until_one_of({ "</think>", "<tool_call>" })) +
                                          p.optional(p.literal("</think>")))
                             : p.eps();
        ...
        if (has_tools && inputs.tool_choice != COMMON_CHAT_TOOL_CHOICE_NONE) {
            ...
            foreach_function(inputs.tools, [&](const json & tool) {
                ...
                tool_choice |= p.rule(
                    "tool-" + name,
                    p.tool_open(p.literal("{\"name\":\"") + p.tool_name(p.literal(name)) +
                                p.literal("\",\"arguments\":")) +
                        p.tool_args(p.json_string()) + p.literal(",\"call_id\":\"") +
                        p.tool_id(p.chars("[a-zA-Z0-9_-]", 1, -1)) + p.literal("\"}"));
            });

            auto min_calls  = inputs.tool_choice == COMMON_CHAT_TOOL_CHOICE_REQUIRED ? 1 : 0;
            auto max_calls  = inputs.parallel_tool_calls ? -1 : 1;
            auto one_call   = "<tool_call>\n" + tool_choice + p.literal("\n</tool_call>");
            auto tool_calls = p.trigger_rule("tool-call", p.repeat(one_call, min_calls, max_calls));
...
std::optional<common_chat_params> common_chat_try_specialized_template(
    ...
    // Millie-native: compact JSON calls with round-tripped call_id inside
    // <tool_call> tags; the template spells the call_id key.
    if (src.find("<tool_call>") != std::string::npos && src.find("\"call_id\":") != std::string::npos) {
        LOG_DBG("Using specialized template: Millie native\n");
        return common_chat_params_init_millie_native(tmpl, params);
    }
