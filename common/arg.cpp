...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--hot-experts"}, "N",
        "resident experts per layer for Metal Qwen3.5 MoE (8-256, 0 disables; default: 0)",
        [](common_params & params, int value) {
            if (value != 0 && (value < 8 || value > 256)) throw std::invalid_argument("hot-experts must be 0 or 8-256");
            params.hot_experts = value;
        }
    ));
    add_opt(common_arg(
        {"--hot-expert-bias"}, "N",
        "additive routing-logit bias for resident experts (default: 1.0)",
        [](common_params & params, const std::string & value) {
            size_t end;
            float bias = std::stof(value, &end);
            if (end != value.size() || !std::isfinite(bias) || bias < 0) throw std::invalid_argument("hot-expert-bias must be finite and nonnegative");
            params.hot_expert_bias = bias;
        }
    ));
    add_opt(common_arg(
        {"--mlock"},
