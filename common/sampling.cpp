...
struct common_sampler * common_sampler_init(const struct llama_model * model, struct common_params_sampling & params) {
    ...
    if (params.ngram_penalty_scale > 0.0f && !params.ngram_penalty_refs.empty()) {
        samplers.push_back(common_ngram_penalty_init(
            params.ngram_penalty_refs, params.ngram_penalty_start_n, params.ngram_penalty_scale, params.ngram_penalty_max));
    }
    ...
    if (params.ngram_penalty_scale > 0.0f && !params.ngram_penalty_refs.empty() && params.backend_sampling) {
        LOG_WRN("%s: backend sampling is not compatible with the n-gram penalty, disabling\n", __func__);
...
// Read-only accounting for the grammar path (LLAMA_GRAMMAR_STATS=1).
// Nothing here changes sampling behaviour.
struct common_grammar_stats {
    std::atomic<int64_t> n_sample{0};       // total sampler invocations
    std::atomic<int64_t> n_grammar_tok{0};  // invocations where a grammar was active
    std::atomic<int64_t> n_resample{0};     // grammar rejected the optimistic token
    ~common_grammar_stats() {
        if (getenv("LLAMA_GRAMMAR_STATS") && n_sample.load()) {
            fprintf(stderr, "\n== grammar: %ld samples, %ld with grammar, %ld resampled (%.1f%% of grammar tokens) ==\n",
                    (long) n_sample.load(), (long) n_grammar_tok.load(), (long) n_resample.load(),
                    n_grammar_tok.load() ? 100.0*n_resample.load()/n_grammar_tok.load() : 0.0);
        }
    }
};
static common_grammar_stats g_grammar_stats;
